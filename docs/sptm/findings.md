...
## [SkR35] 0x004aa1e4-0x004aaee8 — method-dispatch table with empty no-op slots and fixed code-pointer returns
- **Observation**: This 120-entry region is a method-dispatch vtable. A large subset of slots are bare `return;` no-ops (0x4aa38c, 0x4aa3bc, 0x4aa3e4, 0x4aa3fc, 0x4aa42c, 0x4aa44c, 0x4aa464, 0x4aa494, 0x4aa4ac, 0x4aa4c4, 0x4aa594, 0x4aa5ac, 0x4aa5f8, ... 20+ more), several return a fixed constant `0xff` (0x4aa67c, 0x4aa770, 0x4aaad84) or fixed implementation code pointers (0x4aa370 -> 0x6854f0, 0x4aa4e0 -> 0x685578). A slot that silently does nothing or returns a canned sentinel is a classic fail-open surface: a caller that treats a successful return as "handled" would proceed with an unhandled method.
- **Evidence**: decompiles — `void FUN_004aa38c(void){ return; }`, `undefined8 FUN_004aa370(void){ return 0x6854f0; }`, `undefined8 FUN_004aa67c(void){ return 0xff; }`. These are all vtable method slots (region sits in the object-method dispatch area between 0x4aa1e4 and 0x4aaee8; many forward to shared handlers FUN_00469f58/004a3400/00498c84).
- **Severity (hypothesis)**: low — table is a Swift regex/AST object vtable (per slice r34 context), so a no-op slot just means an unimplemented/optional method, not a security boundary; but the `0xff` sentinel and empty returns should be confirmed as intentional "unsupported" stubs, not a dropped implementation.
- **Confidence**: low (no caller that consumes these sentinels was traced).

## [SkR35] 0x004aa9f8 / 0x004aad6c / 0x004aae98 — typed-message send with fixed selector/type words to an out-of-range helper
- **Observation**: FUN_004aa9f8 invokes the message sender sk_x_0046c4e0 with fixed selector/type words `0x22, 0xe100000000000000, 0x225c, 0xe200000000000000, 0x7fffffffffffffff` plus the two caller-supplied words. The caller-controlled words (param_1/param_2) are forwarded verbatim into the send path; the 0x7fffffffffffffff mask and the 0xe1/0xe2 tagged words are the only bounds/type qualifiers. FUN_004aad6c/004aae98 likewise pass caller args straight into sk_x_004a4ac4 with fixed metadata pointers (0x657de8/0x5a4bd8). If the receiving capability is attacker-addressable, the raw word forwarding is the trust boundary.
- **Evidence**: decompile 0x4aa9f8 — `FUN_0046c4e0(0x22, 0xe100000000000000, 0x225c, 0xe200000000000000, 0x7fffffffffffffff, param_1, param_2)`; 0x4aad6c/0x4aae98 — `FUN_004a4ac4(param_1, ..., &DAT_00657de8, &DAT_005a4bd8)`.
- **Severity (hypothesis)**: low — fixed-qualifier IPC send; the caller words are data, and the tagged words pin the operation type. No independent validation of the forwarded words in the stub itself.
- **Confidence**: low (the callee 0x46c4e0 was not fully traced).

## [SkR37] 0x4ab7f0-0x4ac088 — embedded Swift-runtime helper cluster: ~50 no-op thunk slots + a left-in "// TODO: Atom" debug string literal
- **Observation**: This 120-entry region is a dense cluster of tiny runtime helpers: ~50 bare `return;` no-op witness/thunk slots (0x4ab800, 0x4ab810, 0x4ab820, 0x4ab844, 0x4ab890, 0x4ab89c, 0x4ab8dc, 0x4ab8e8, 0x4ab910, 0x4ab920, 0x4ab930, 0x4ab968, 0x4ab998, 0x4ab9bc, 0x4ab9e0, 0x4ab9ec, 0x4aba28, 0x4aba34, 0x4aba88, 0x4aba94, 0x4abacc, 0x4abad8, 0x4abb1c, 0x4abb28, 0x4abbac, 0x4abbb8, 0x4abbd4, 0x4abc0c, 0x4abc50, 0x4abc6c, 0x4abcdc, 0x4abcf8, 0x4abd04, 0x4abd20, 0x4abd38, 0x4abd90, 0x4abd9c, 0x4abdb8, 0x4abdc4, 0x4abe24, 0x4abe74, 0x4abe90, 0x4abea0, 0x4abeb0, 0x4abed0, 0x4abf10, 0x4abf20, 0x4abf30, 0x4abf40, 0x4abf6c, 0x4abf7c, 0x4abfb8, 0x4abfc8, 0x4abfd8, 0x4ac008), plus fixed-size value-witness struct copies (0x4ab868, 0x4ab954, 0x4abb64, 0x4abe40...), tagged-pointer helpers (0x4abd2c, 0x4abf00, 0x4abf50), fatal trampolines (0x4ab944, 0x4abe5c, 0x4abb8c), and a copy-and-retain group (0x4aba50/0x4aba60/0x4aba70/0x4abbc4/0x4abbe0/0x4abda8). FUN_004ab9f8 returns the raw string literal "  // TODO: Atom \xef…" (0x4f444f54202f2f20/0xef206d6f7441203a), an embedded Swift source-comment placeholder.
- **Evidence**: decompiles — 50+ `void FUN_004abXXX(void){ return; }`; `auVar1._0_8_ = 0x4f444f54202f2f20; auVar1._8_8_ = 0xef206d6f7441203a;` at 0x4ab9f8; fatal string " in range" at 0x4abb8c via thunk_FUN_002acbb8.
- **Severity (hypothesis)**: low — no-op slots are likely unimplemented/optional value-witness or method-dispatch slots (consistent with the surrounding Swift metadata engine per slice R01 context), and a fail-open caller that treats an empty return as "handled" is the only theoretical surface. The "// TODO: Atom" string is developer instrumentation shipped in the binary, not a live trust boundary; it is noted for completeness (indicates generated metadata kept a placeholder name).
- **Confidence**: low (no caller consuming the empty-return slots was traced; no attacker-controlled operand found in any body).

## [SkR36] 0x004aaf08-0x004ab7e0 — large run of empty no-op method-dispatch/witness slots
- **Observation**: Of the 120 functions in this slice, ~67 are bare `void FUN(void){ return; }` bodies (). A silent no-op in a method-dispatch table is a classic fail-open surface: a caller treating a successful return as "handled" would continue past an unhandled/optional method. This mirrors the SkR35 observation on the adjacent vtable region.
- **Evidence**: decompiles — `void FUN_004aaf08(void){ return; }`, `void FUN_004ab034(void){ return; }`, `void FUN_004ab7cc(void){ return; }` (all identical empty bodies); region sits in the Swift regex/AST object-method dispatch area between 0x4aaf08 and 0x4ab7e0, many siblings forward to shared helpers (FUN_0036b270 teardown, FUN_00365b6c release, FUN_00002534 fatal-error).
- **Severity (hypothesis)**: low — consistent with an optional/unsupported-method vtable for Swift RegexBuilder AST nodes (per slice r34 context), not a security boundary; but the empty slots should be confirmed as intentional, not a dropped implementation.
- **Confidence**: low (no caller consuming these sentinels was traced).

## [SkR36] 0x004aaf6c / 0x004ab630 — Swift fatal-error message builders with string+metadata pairs
- **Observation**: FUN_004aaf6c and FUN_004ab630 each invoke the fatal-error builder FUN_00002534 with a string-table pointer (0x657bb8 / 0x657c08) and a metadata address (DAT_005a3c60 / DAT_005a3cb0). These are reachable failure/assertion paths that terminate the microkernel region.
- **Evidence**: decompiles — `FUN_00002534(0x657bb8, &DAT_005a3c60)` and `FUN_00002534(0x657c08, &DAT_005a3cb0)`.
- **Severity (hypothesis)**: low — message strings are static; the builder is the Swift runtime fatal-error routine.
- **Confidence**: medium (strings + metadata addresses match the Swift `_fatalErrorMessage` shape).

## [SkR36] 0x004ab5ec — teardown index computed from register residue
- **Observation**: FUN_004ab5ec calls the teardown helper FUN_0036b270 with the expression `(x9 + 0x10) & (x9 ^ ~0UL)`, an index derived from the register-global x9. The callee is invoked with a register-derived argument rather than a validated constant.
...
## [SkR41] 0x004b23d8 sk_vas_page_release — refcount-underflow surface on the region freelist path
- **Observation**: The slice's only non-trivial function releases a 12-bit page tag by clearing its present bit in a per-region bitmap (`reg + (off>>12)`, `bit = 1 << (off>>6 & 0x3f)`), decrementing a 16-bit refcount word at `reg+4`, and pushing the region into a global freelist (`glob[6]`) only when the refcount is above the 0xff reserve threshold. Two invariant guards (`[VAS abort]` panics) fail closed when the bit is absent or the offset exceeds 0x3fff. The refcount decrement `*(uint16_t*)count = rc - 1` is NOT guarded against `rc == 0`; if any present page is ever released while its region refcount is already 0, the 16-bit word underflows to 0xffff (and the `rc > 0xff` branch would then wrongly push the region onto the freelist). The whole op sits inside a 16-byte transaction (`FUN_00034bd8` / `FUN_00034d5c`) with a `do { CallSupervisor(4); } while (tag==1)` spin.
- **Evidence**: decompile 0x004b23d8 — `if ((*bitmap & bit)==0) vas_abort(0x5b199d); *bitmap &= ~bit; rc=*count; if (0xff < (ushort)rc){ push freelist } *(ushort*)count = rc-1;` — the decrement executes unconditionally after the bit-clear, with no `rc!=0` precheck. `0x3fff` offset bound via `if (0x3fff < off) vas_abort(0x5b1937)`.
- **Severity (hypothesis)**: low-medium — relies on the invariant that a present page always has refcount>=1, which the "bit absent => abort" guard enforces unless the bitmap and refcount can be desynchronized; a guest-triggerable desync would give a 16-bit underflow + spurious freelist push (heap reuse of a live region).
- **Confidence**: medium (single source; ownership/refcount discipline across the wider VAS layer not yet traced).

## [SkR41] 0x004b20e8 sk_security_assert_fail + 0x004b1c84 read-only-map enforcement — VAS layer uses fail-closed security assertions
- **Observation**: The slice contains a dedicated `Security assertion failed: %s, %f` abort handler (0x4b20e8 via FUN_001150e0) and an `unable to map read only` reporting stub (0x4b1c84), alongside 40+ `[VAS abort in function '%s' at line %d]` panic stubs (FUN_004afae4) covering the vas-core / heap / siptable layers. Together they show the ring −1 VAS mapper fails closed on invariant and security-assertion violations rather than returning.
- **Evidence**: string refs 0x005a8b30 "Security assertion failed: %s, %f" (0x4b20e8), 0x005b00bf "%s(%zu): unable to map read only" (0x4b1c84); 43 direct `FUN_004afae4(fmt)` abort stubs each with a distinct source line string.
- **Severity (hypothesis)**: low — fail-closed panics are correct hardening; but each is a microkernel fault (DoS) on any guest-influenced input reaching the assertion, so the triggers (not the handlers) are the surface to audit.
- **Confidence**: high (string + structure confirmed).

## [SkR41] 0x004b10c0-0x004b1a50 — TB (tightbeam) codec / Swift-runtime fatal traps are unconditional, no recovery
- **Observation**: 60+ near-identical trap thunks (stride 0x24) each print a TB_FATAL / runtime fatal message (sources: `tb_codec.c`, `/AppleInternal/Library/BuildRoot/...`) then execute `brk #1` unconditionally. They are compile-time-inserted fatal-error sites (Swift `_fatalError`-style), not reachable-with-input failure branches.
- **Evidence**: each body = `FUN_00019800/810/820/830/840|00015e2c|000179d8(file_string); SoftwareBreakpoint(1, next);` with fixed file strings; no parameters, no data-dependent branches.
- **Severity (hypothesis)**: low — static traps, no attacker input; informational (they map the tightbeam codec's overflow-during-decode/encode trap surface).
- **Confidence**: high.

## [SkR40] 0x004afbc4-0x004b109c — dense cluster of fail-closed abort shims (TB_ASSERT/TB_FATAL/malloc_assertion/XRT/security)
- **Observation**: 114 of 120 functions in this slice are deterministic fail-closed abort stubs. Each calls a cL4 diagnostic reporter (FUN_00118b28 TB_ASSERT/TB_FATAL; FUN_00004cc0 malloc_assertion; FUN_00115424 XRT thread assertion; FUN_001150e0 security assertion; FUN_000179c8-9f8 / FUN_00019800-840 cL4_large_transport / tb_codec reports) with a fixed message string, then executes a SoftwareBreakpoint(1,addr) trap (rendered CL4_SWBP). None return. The distinct abort sites are: 004afbc4 "Unexpected L4 Error", 004afc1c "Security assertion failed", 004afc5c-004b0018 malloc assertions, 004b0034-004b0744 (TB_ASSERT/TB_FATAL spanning td->slots, tb_message_get_state/disposition, new_slot==L4_Nil, overflow-detected) plus 004b01e8 XRT thread tss_get assertion.
- **Evidence**: decompiles show each body is `report(msg); SoftwareBreakpoint(1, next_addr);` with noreturn warnings; e.g. 004b0034 `FUN_00118b28("TB_ASSERT: td->slots..."); SWBP(0x4b0068)`; 004b0570/004b0690/004b06e4/004b0714 all share `"TB_FATAL: overflow detected when..."`; 004b0850-004b109c form a run of 20+ `tb_codec`/`cL4_large_transport` /AppleInternal-build-root assertion sites (strings s_tb_codec_c_005ac712, s_cL4_large_transport_c_005ac025).
- **Severity (hypothesis)**: low-medium — these are defensive fail-closed traps: hostile/garbage input reaching any of these invariants kills the microkernel region (a ring −1 DoS) rather than corrupting memory. The density (~114 traps across one tightbeam/large-transport slice) indicates the transport/codec path is heavily assert-protected; a bypass of any single check is the surface to watch.
- **Confidence**: high (deterministic abort pattern; string-consistent).

## [SkR40] 0x004afd3c — large-transport descriptor decoder without explicit bound on element stride
- **Observation**: 004afd3c builds a result descriptor with element count (node[0x100]>>14) and per-element 0x60-byte entries, iterating while `i < node[0x105]`. The element count comes directly from a node field and sizes an allocation via FUN_00002e50; if node[0x105] (a byte count) can exceed the number of slots backed by the allocation, the +0x60-strided writes at offsets 0x40/0x44/0x48 would walk past the buffer.
- **Evidence**: decompile: `count=*(uint*)(node+0x100)>>0xe; result=FUN_00002e50(buf,10,0,count*node[0x105],0,node[0x100],0,0); do { *(e+0x40)/0x44/0x48 } while (i<node[0x105])` with `off += count*0x60` each iteration.
- **Severity (hypothesis)**: medium — potential out-of-bounds descriptor write if node[0x105] is inconsistent with the allocated `count*node[0x105]` size; the allocation size equals the loop bound times the element count, so a mismatch between the `count` used for allocation and the value used for the stride would be the bug. No caller traced to confirm the fields are validated upstream.
- **Confidence**: medium (register-artifact body; decode semantics inferred).

## [SkR40] 0x004b0744 — supervisor spin loop + indirect vtable dispatch
- **Observation**: 004b0744 spins on `CallSupervisor(4)` while a status word is 1, then dispatches through two indirect (blraa-authenticated) calls: `*(vtbl+0x38)(base, *param_1, 0, 0)` (x17=0xfbee) and `*(*param_1)()` (x17=0x9659). The vtable pointer pair comes from FUN_00034a2c. Target provenance is entirely register-carried, so the dispatch targets are not statically verifiable from this slice.
- **Evidence**: disassembly: `svc 0x4; cmp x0,#1; b.eq loop`; `ldr x9,[x1,#0x38]; blraa x9,x17=0xfbee`; `ldr x8,[x1]; blraa x8,x17=0x9659`; `b 0x125b0` tail.
- **Severity (hypothesis)**: low-medium — a supervisor-call spin that blocks until an external condition clears, followed by guest/class-method dispatch; if `*param_1` or the vtable is attacker-influenced the indirect calls are an arbitrary-call surface, but the loop and dispatch sit inside the tightly-guarded transport layer.
- **Confidence**: medium.

## [SkR40] 0x004afe60 — span resize arithmetic with register-artifact index
- **Observation**: 004afe60 rounds a requested size to a 0x4000 granule, derives a chunk index from `((data - base[0x38]) + ((node-base-0x50)/0x60)*-0x4000) / cap`, and writes the new size byte at `node + (idx&0xff)*(cap>>14)*0x60 + 0x23` before memmoving the trailing payload (FUN_00003788). The `idx&0xff` masking limits the index to 256 slots; the chunk-size field is a single byte.
- **Evidence**: decompile shows the `&0xff` index mask and byte-width size field; `FUN_00003788(base[0x10], data+size, old-(size>>14))` memmoves trailing data on shrink.
- **Severity (hypothesis)**: low — byte-size fields cap the reachable span; index masking bounds the slot; the memmove length is signed (old - new) but only reached when new<old, so a shrink-only move. Low risk absent a signedness flaw in the caller-supplied size.
- **Confidence**: medium.

## [SkR43] 0x004b4424-0x004b5744 — dense TightBeam (TB) server-dispatch assert/fatal trap table (120 sites)
- **Observation**: This contiguous 120-function region is a table of noreturn trap thunks emitted for `TB_ASSERT`/`TB_FATAL` checks in the TightBeam server-dispatch layer (libvas build, source file `EASM_C.c`). Two distinct failure families are exposed as literal message strings: (1) vtable-slot-NULL asserts `TB_ASSERT: (server-><method> != ((void*)0)) && "implementation for <Method> is not present"` covering the full TightBeam server method surface (physicalAddress, frameCopy, rootCopy, handleFault, spanAt, allocEndpoint, allocExecutionContext/AMXContext/SMEContext, freeKernelObject, faulthandlerCreate/StartFault/CompleteFault/Destroy, spanAlloc/Merge/Split/Resize/Destroy/Config/Downgrade/Bump/Populate/Depopulate/MapFrame/SetFaultstate, setupSuccessful, mapMacho, registerThread(+ExecutionContext)/unregisterThread, dyldMapRegion, getSegmentInfoTable); and (2) error-return fatals `TB_FATAL: invalid error returned from <method>`. A third family is the capability-decode assert `tb_message_decode_capability(msg,...) == TB_ERROR_SUCCESS && "failed to decode capability"`.
- **Evidence**: decoded message strings at 0x005b854a..0x005ba950 (read_memory, exact text above); each thunk = `FUN_0004b478/488/454/00118b28(msg)` then `SoftwareBreakpoint(1, next)`. The four printers: 0x0004b478 ("unexpected tb_error_t returned"), 0x0004b488 ("completion block must be called before returning"), 0x0004b454 ("invalid value: unexpected case value"), 0x00118b28 (generic message report).
- **Severity (hypothesis)**: low — these are fail-closed defensive traps: a NULL TightBeam server vtable slot or an unexpected error return kills the microkernel region (ring −1 DoS) rather than allowing a NULL/erroneous dispatch. The density documents that every TightBeam server method is NULL-checked before indirect dispatch; the security surface is the *absence* of such a check or a path that reaches these with attacker-controlled state, not the traps themselves.
- **Confidence**: high (deterministic abort pattern; all messages string-matched).

## [SkR44] 0x004b5774-0x004b81e8 — dense fail-closed abort/trap table (TB_FATAL / pmm / TSS / ipmm / list invariants)
- **Observation**: ~90 of 120 functions in this slice are deterministic noreturn abort/trap thunks. The TB layer uses `FUN_00118b28("TB_FATAL: invalid error returned")` + `SoftwareBreakpoint(1, next_addr)` (17 sites, 0x4b5774-0x4b5a44). PMM asserts go through `FUN_004afae4` (set_self-twice/invalid, alloc_stream, alloc_slots, MAX_COW_REGIONS, pmm_server_already_set, no_cnodes, bad_slot_layout, boot_slot_reserve, create_object). A second family uses the `FUN_0005b1b0(0, fmt, ...)` "Unexpected L4 Error" printer (0x4b5c0c, 0x4b5de8, 0x4b5ef8, 0x4b5f70, 0x4b7a90, 0x4b8054). A third family is a uniform `assert_fail_report` template: `err_ctx_save(); sched/entropy/tss/ipmm/list_ctx_save(); panic_report_context(); ctx_restore(); FUN_004afae4("Unexpected L4 Error...")` (0x4b6938..0x4b81e8). Security assertions (`FUN_001150e0` "Security assertion failed") at 0x4b5f30/0x4b6864/0x4b808c. None return.
- **Evidence**: decompiles each show `report(msg); SoftwareBreakpoint(1, next);` with noreturn warnings; e.g. 0x4b5774 `FUN_00118b28("TB_FATAL: invalid error returned"); SWBP(1,0x4b57a4)`; 0x4b5aa0 `FUN_004afae4("pmm: set_self: called multiple times")`; 0x4b6938 full assert template above.
- **Severity (hypothesis)**: low — fail-closed defensive traps; hostile input reaching any invariant kills the microkernel region (ring −1 DoS) rather than corrupting memory. The security surface is the *trigger* paths (pmm/TSS/ipmm state machines), not the handlers.
- **Confidence**: high (deterministic abort pattern; string-consistent).

## [SkR44] 0x004b5fa8 / 0x004b5fb8 — shared sync-trace pool allocator with atomic counter and free-list walk
- **Observation**: Both functions allocate 0x40-byte entries from a single global pool (FUN_0005b120(0x6b04b8)) for the current thread / a given thread. When the free-list head (pool+0x58) is empty, they bump an atomic counter (pool+0x50, LOAcquire/LORelease) and index the entries array (pool+0x40) by `idx*0x40`; on exhaustion they decrement the counter, log "xrt: sync trace pool exhausted", set pool state=3, and return 0. A debug flag at thread+0x48 bit-4 gates re-entrancy. The counter is only bounded by `idx < pool[0x28]` after the refill path; the free-list pop (`*head=*slot; *slot=NULL`) does not re-check `idx` against the backing array size on the free-list path, and every corruption check falls to `SoftwareBreakpoint(0x5519,0x4b6194)`.
- **Evidence**: decompile: `puVar3=FUN_0005b120(0x6b04b8); plVar7=*(long**)(puVar3+0x16); if(plVar7==0){LOAcquire();uVar4=*(ulong*)(puVar3+0x14);*(puVar3+0x14)=uVar4+1;LORelease();...} else { *(puVar3+0x16)=*plVar7; *plVar7=0; }` then 0x40-byte entry init and free-list relink.
- **Severity (hypothesis)**: medium — the pool is shared across threads with only the counter atomic; if the trace pool is reachable from guest-controlled IPC volume, an exhaustion or a race between the free-list pop and the refill path could yield an uninitialized/aliased entry. The 0x40-byte stride and `idx*0x40` indexing are attacker-agnostic (pure counter), so an overflow would require the counter to exceed `pool[0x28]`, which is checked on the refill path only.
- **Confidence**: medium (register-artifact body; exact trigger provenance not traced).

## [SkR44] 0x004b6194 — held-object release with fail-closed unlink and synchronisation-object carve-out
- **Observation**: `sk_release_held_object(owner,target)` walks the held-object list at owner+0x60 to find `target` (node[1]), unlinks it, returns the entry to the shared pool free-list, and panics on three distinct violations: "release_unheld_object" (target not in list), "released outer object while hold" / "released" (list-order invariant), and "released synchronisation object" (a sync object released via the wrong path — must go through the sync tracer). A companion (0x4b63cc) walks the same list at thread exit and panics "exited returned while holding" if anything is still held.
- **Evidence**: decompile: `do { plVar9=(long*)*plVar5; ... } while (plVar9[1]!=param_2);` with `FUN_0005ae4c(owner,node,"released outer object while hold",flag)`, `s_released_synchronisation_object_o_005bc2dd`, `s_exited_returned_while_holding_005bc349`; corruption -> `SoftwareBreakpoint(0x5519,0x4b63ac/0x4b6430)`.
- **Severity (hypothesis)**: medium — the fail-closed paths (double-release, foreign release, sync-object misrelease, exit-with-held) are exactly the invariants that would otherwise produce a use-after-free / leak in a held-object cache. The synchronisation-object carve-out (0x4b6194 skips the sync path when pool flag bit1 set) is the subtle one: if a thread can mark an object as synchronisation-held without the matching sync-tracer release, the entry is dropped from the held list without a corresponding free. Worth tracing callers.
- **Confidence**: medium (list-walk logic faithful; caller provenance not traced here).

## [SkR44] 0x004b6d60 — TCB destroy path ends in authenticated indirect vtable dispatch
- **Observation**: `sk_tcb_destroy(tcb)` releases two wait objects (refs 0x15/0x14) and the master object (ref 6), drains the pending scheduling flag (CallSupervisor(4/5) spin), releases the bound scheduler TCBs, then dispatches `(*(vtbl+0x38))(instance+0x30)` where vtbl is `**(tcb+0x38)` and instance is `*(tcb+0x30)`. Target provenance is entirely register-carried from the TCB, so the destroy method is not statically resolvable from this slice.
- **Evidence**: decompile tail: `(*(code*)**(uint64**)(tcb+0x38))(*(uint64*)(tcb+0x30));` after `FUN_0019ae60(x,0x15/0x14/6)` refcount drops and `CallSupervisor` spins on tcb+0x88/0x70.
- **Severity (hypothesis)**: low-medium — if a guest can corrupt a TCB's destroy vtable/instance pointers (e.g. via a stale cap or a partial free), the indirect call becomes an arbitrary-call in the kernel's object-destroy path. The refcount drops (refs 0x15/0x14/6) happen before the dispatch, so a dangling object could reach it; worth confirming the vtable/instance fields are only written at TCB init.
- **Confidence**: medium.

## [SkR44] 0x004b6430 — monitor error-code decode switch (register-artifact)
- **Observation**: `sk_fault_retry_report` re-enters a faulted IPC context and decodes the monitor return status into the L4 error-code strings (SlotInvalid/MethodInvalid/ArgumentInvalid/OperationInvalid/PermissionInvalid) via a `switch(status)` that selects only a log string, then falls into the panic-report + stack-canary check. The decompiler rendered the status decode entirely through `extraout_*` register artifacts (0x004b6430-0x004b67b4), so the exact comparison order is not recoverable cleanly.
...
## [SkR46] 0x0064ebcc / 0x0064de38 / 0x0064de64 — L4 IPC + page-map primitives drive the GL0 supervisor interface via CallSupervisor
- **Observation**: The 0x64 region is the cL4 kernel's embedded Swift runtime "Prespecializations" bootstrap plus its L4 IPC glue. The core call primitive (0x4ebcc) packs a message descriptor `(method<<16 | wordcount & 0x3f)` plus up to 8 word arguments into the per-cpu message buffer (tpidrro_el0) and traps `CallSupervisor(0)`. The page-map helpers (0x4de38/0x4de64) build a 0x11-tagged page-map request from prespecialization-metadata-derived addresses and issue the same trap, then iterate page slots and call a completion callback (offset +0x28 in the runtime context). No capability/validity check on the addresses is visible in this slice — the addresses come from the metadata records and are passed straight to the GL0 interface.
- **Evidence**: decompile (0x4ebcc): `*puVar4 = (uVar17<<0x10)|param_3&0x3f; CallSupervisor(0); *puVar4 = ...;` and (0x4de38): `*puVar1 = 0x11; puVar1[1] = uVar9<<0xe; puVar1[2] = lVar5; CallSupervisor(0);`.
- **Severity (hypothesis)**: low (informational) — the supervisor interface itself is the trusted GL boundary; the kernel-side writer of the message buffer is the caller. The main follow-up is confirming the prespecialization metadata that supplies page addresses is trusted kernel .data (it is boot-time Swift metadata in the kernel image) rather than guest-controlled.
- **Confidence**: medium (CallSupervisor ABI rendering unambiguous; metadata provenance not traced from this slice).

## [SkR46] 0x00650b3c / 0x00650af8 / 0x00650ed0 — record iteration validates kind/element indices but element pointer arithmetic depends on a size table
- **Observation**: `sk_meta_record_element` (0x50b3c) bounds the record kind to `< 0x1d`, multiplies the element index by the per-kind element size from the 0x6b5f28 table, and returns 0 when the size is 0 or the offset is out of range; `sk_meta_record_base` (0x50af8) rejects an index with `idx>>0x3a != 0` and bounds `idx*0x40` against the block size. `sk_meta_record_iter_next` (0x50ed0) validates each record header/size field before advancing. These are consistent defensive checks.
- **Evidence**: decompile (0x50b3c): `if (0x1c < (uint)uVar3) FUN_0065c2f0(0,0x6a592b); ... if (SUB168(auVar1*auVar2,8)==0 && param_2*uVar3 < count) return (long)param_1+param_2*uVar3+0xc;` (0x50af8): `if (param_2 >> 0x3a != 0) FUN_0067b280(0x6a576b); if (param_2*0x40 < (ulong)param_1[1]) ...`.
- **Severity (hypothesis)**: low (informational) — the checks close the obvious OOB paths; the only residual is that the element-size table (0x6b5f28) and the record kind/size fields themselves are data that, if ever attacker-controlled, would set the stride used for the bounds check (self-referential bounds). Input is boot-time kernel metadata.
- **Confidence**: medium (bounds logic clear; table provenance untraced).

## [SkR46] 0x004ba214 — symbolic-reference resolver traps fail-closed when the fixup marker is not 0x01
- **Observation**: `sk_meta_symbolic_ref_resolve` walks the runtime fixup list, but the decompiler emitted two `SoftwareBreakpoint(1,0x4ba2e4)` noreturn traps for the "fixup marker byte != 0x01" cases, then a final `FUN_00369bb0(0,"Failed to look up symbolic reference")` abort on the success-but-unresolved tail. All resolution-failure paths terminate the kernel rather than returning a recoverable error.
...
## [ringminus1] 003cccf4-003d2e80 (SKR55) — cL4 IPC message-register (MR) tag emitters + demangler/map
- **Observation**: This 120-function slice is the second half of the cL4 message-register packer (sibling to SKR05/SKR06). The bulk are per-type tag emitters (`cL4_ipc_emit_*`) that decode a message descriptor word span and re-emit it into the per-TCB outbound MR buffer at `ctx+0x2140` (depth/state word at `ctx+0x2150`) as a short token (e.g. "MB","TT","Tl","WOe") followed by decoded fields; the tag's word count is taken directly from the descriptor's length field (e.g. `cL4_ipc_emit_te` @003cd53c passes `msg[1]` as the count to the emit helper). The central dispatcher `cL4_msg_dispatch_full` (003d08c4) bounds recursion to depth 0x400 (error 3/code 0x223), validates descriptors via `cL4_msg_kind`/`cL4_msg_header`, and gates unknown kinds to error 5/code 0x24d. The slice also contains the demangler node-kind name table lookup (`cL4_demangle_kind` 003d2b1c) and the internal hash map (`cL4_map_*` 003d2740-003d28d8).
...
## VB2_7 sweep findings (VerifyV7 subagents)

- [VB2_7/syscalls] 0x0037dc2c sk_desc_assign / 0x00384740 cL4_obj_copy5: indirect calls through vtable pointers loaded from descriptor tag metadata (*(type-8)+{0x10,0x18,0x20,0x28,0x8}), gated only by the +0x52 inline-flag bit. A caller-passed attacker-influenced descriptor tag yields a controlled function-pointer call. Severity: Medium. Confidence: high (structure verified).
- [VB2_7/syscalls] 0x00372a44 sk_desc_copy: allocates 0x58 and copies 11 words from *(param_1-8) with no bounds check on param_1 — attacker-controlled param_1 is an OOB read feeding an object copy. Severity: Low-Medium. Confidence: high.
- [VB2_7/syscalls] 0x0037ca78 sk_meta_flatten_counts: reads *(char**)(param_2+4) for param_2[0] bytes and *(u32**)(param_2+8) for param_2[1]*0xc bytes — counts-driven pointer walk; oversized counts over-read. Severity: Low. Confidence: high.
- [VB2_7/r71] 0x006825d0 sk_free_list_insert: allocator free-list coalesce derives all written pointers from node-supplied {next,len}; a forged/overlapping block could splice arbitrary pointers into the free list -> arbitrary write through the allocator. Severity: high. Confidence: medium (block metadata layout estimated).
- [VB2_7/r72] 0x00685234 sk_r72_685234_fault_header: FUN_00663c04 copies with length == the object-type tag from obj+0x28 into a 32-byte stack buffer — a large tag over-reads the stack on a fault/panic path. Severity: low (panic-path). Confidence: medium.
- [VB2_7/caps] 0x000b3cfc sk_htbl_finish: hash-table rebuild dispatches a hash function pointer from a source object at +0xd8 and stores that object pointer into the new table — indirect-call + pointer-store primitive if source slots are attacker-influenced. Severity: Low. Confidence: High.
- [VB2_7/caps] 0x000b3cfc (caller 0xb3b88 sk_htbl_build): the caller discards the returned table — correctness concern in the sibling transcription. Severity: Informational.
- [VB2_7/vspace] 0x002676bc sk_vspace_object_create: capability/vspace-object install path — tagged-object bit dispatch, 48-bit address mask, cap resolve + install, then msg dispatch. Security-relevant object-install path; structure + arg-forwarding verified. Severity: info. Confidence: medium.
- [VB2_7/slice189] 0x0040eac8 sk_cont_result_dispatch: continuation-record metadata indirect dispatch `(**(code**)(*(rp[2]-8)+0x38))(value,flag,1)` + +0x10 variant — code-pointer dispatch on attacker-influenceable record contents. Severity: low-medium. Confidence: medium.
- [VB2_7/boot] 0x0004c574 sk_ep_call: supervisor-boundary marshalling via tpidrro_el0 — args copied into TLS window +0x1e0..0x200 under explicit bounds checks; selector word written to TLS[0] and restored around CallSupervisor(0). Bounded copy prevents OOB into the window. Severity: info. Confidence: high.
- [VB2_7/boot] 0x0002db3c/0x0002e2bc vas_fh_map/clear: ring-slot pointer arithmetic (next % count)*6 into base+0x200 now explicitly bounds-checked; resolved page released via sk_tcb_slot_release; unknown vas return codes abort. Severity: info. Confidence: high.
