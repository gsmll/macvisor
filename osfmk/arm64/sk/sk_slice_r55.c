...
void cL4_msg_arg_index(int *result, unsigned long tcb, unsigned long *msg, unsigned long idx, unsigned long depth); /* FUN_003d0280 */



/* ==================================================================== *
 * SKR55 slice: 120 functions in address order.  Bodies transcribed from
 * program cl4_kernel.raw (cL4 Secure Kernel). 67+ of these were originally
 * written by the SKR06 worker into sk_slice_r06.c for the same region; they
 * are re-emitted here verbatim so this slice is self-contained.  The 3 that
 * were missing there (003cd484, 003cd618, 003d08c4) are transcribed here.
 * ==================================================================== */
...
/* 003cd484 @ 0x003cd484   (est. cL4_ipc_emit_tl2)
 * Ghidra: void FUN_003cd484(int *result, long tcb, undefined8 msg, int depth)
 * Decode every argument in `msg` as a list and emit it as a tagged IPC
 * message-register record with tag "Tl" (2 words); on a parse error leave the
 * result word set, otherwise clear the 5-word result. Confidence: medium. */
void cL4_ipc_emit_tl2(int *result, unsigned long tcb, unsigned long msg, unsigned long depth)
{
    cL4_msg_args(result, tcb, (unsigned long*)msg, depth + 1);
    if (*result == 0) {
        cL4_mr_emit_tag(tcb + 0x2140, "Tl", 2, *(unsigned long *)(tcb + 0x2150));
        *result = 0; result[2] = 0; result[3] = 0; result[4] = 0;
    }
}
...
/* 003cd618 @ 0x003cd618   (est. cL4_ipc_emit_woe)
 * Ghidra: void FUN_003cd618(int *result, long tcb, undefined8 msg, int depth)
 * Decode every argument in `msg` as a list and emit it as a tagged IPC
 * message-register record with tag "WOe" (3 words); on a parse error leave the
 * result word set, otherwise clear the 5-word result. Confidence: medium. */
void cL4_ipc_emit_woe(int *result, unsigned long tcb, unsigned long msg, unsigned long depth)
{
    cL4_msg_args(result, tcb, (unsigned long*)msg, depth + 1);
    if (*result == 0) {
        cL4_mr_emit_tag(tcb + 0x2140, "WOe", 3, *(unsigned long *)(tcb + 0x2150));
        *result = 0; result[2] = 0; result[3] = 0; result[4] = 0;
    }
}
...
/* 003d08c4 @ 0x003d08c4   (est. cL4_msg_dispatch_full)
 * Ghidra: void FUN_003d08c4(int *result, undefined8 *tcb, long *msg, undefined8 depth)
 * Top-level message dispatcher. Guards against unbounded recursion (depth
 * > 0x400 -> error 3, code 0x223). For a simple descriptor (cL4_msg_kind==0)
 * selects a type tag by the embedded kind word (0x19 "CKT", 0x3f "OWAW",
 * 0xb1 "XYSg" x2, 0xe7 "VvW", 0xf6 "abd") and routes through
 * cL4_msg_dispatch_tag; kind 0xf5 is a computed call through the TCB's
 * word 0x434 (a function pointer) whose result is decoded via cL4_arg_decode.
 * For a composite descriptor it opens a format record (cL4_fmt_begin /
 * cL4_msg_header), recurses on the payload, emits a packet byte 0x79 via
 * cL4_msg_dispatch_pkt, decodes a 3-element array of argument words, then
 * closes with a 0x47 byte and cL4_out_commit. Unknown kinds set error 5,
 * code 0x24d. Confidence: medium (tags via DAT refs). */
void cL4_msg_dispatch_full(int *result, unsigned long *tcb, unsigned long msg, unsigned long depth)
{
    unsigned int depth_u = (unsigned int)depth;
    unsigned long uVar4; int kind; unsigned short kword;
    unsigned int hlo, hhi; unsigned long hw1, hw2, hw3;
    unsigned char desc[16] = {0}; unsigned char byte;
    const char *tag; unsigned long tagc;
    ...
    kind = cL4_msg_kind((long)msg);
    if (kind == 0) {
        kword = *(unsigned short *)(msg + 2);
        if (kword < 0xe7) {
            if (kword == 0x19) { tag = "CKT"; }
            else {
                if (kword != 0x3f) {
                    if (kword == 0xb1) { tag = "XYSg"; tagc = 2; goto tag_dispatch; }
                    goto bad_kind;
                }
                tag = "OWAW";
            }
        } else if (kword == 0xf6) { tag = "abd"; }
        else {
            if (kword == 0xf5) {
                ...
                uVar4 = cb(tcb[0x435], 0, *(unsigned long *)msg);
                cL4_arg_decode(result, (unsigned long)tcb, uVar4, depth_u + 1);
                return;
            }
            if (kword != 0xe7) goto bad_kind;
            tag = "VvW";
        }
        tagc = 1;
    tag_dispatch:
        cL4_msg_dispatch_tag(result, (unsigned long)tcb, msg, (unsigned long)tag, tagc, depth);
        return;
    bad_kind:
        *result = 5;
        *(unsigned long **)(result + 2) = (unsigned long*)msg;
    ...
    /* composite descriptor: open a format record */
    if (cL4_fmt_begin((unsigned long)tcb, msg, (unsigned char(*)[16])desc, 0) != 0) goto success;
    cL4_msg_header((int*)&hlo, (long)msg, tcb[0]);
    if (hlo != 0) {
        *(unsigned long *)(result + 2) = hw1;
        *(unsigned long *)result = ((unsigned long)hhi << 32) | (unsigned long)hlo;
        *(unsigned long *)(result + 4) = hw2;
        return;
    }
    cL4_msg_dispatch_full(result, tcb, hw3, depth_u + 1);
    if (*result != 0) return;
    byte = 0x79;
    cL4_msg_dispatch_pkt(result, (unsigned long)tcb, msg, &byte, depth_u + 1, 0);
    if (*result != 0) return;
    if ((*(char *)((long)msg + 0x12) != 5) || ((int)((unsigned long *)msg)[1] != 3)) goto tail;
    {
        unsigned long *elems = *(unsigned long **)(*(unsigned long *)msg + 0x10);
        unsigned char eb = *(unsigned char *)((long)elems + 0x12);
        unsigned long n = eb;
        unsigned long i;
        if (eb == 1) {
        arr_loop:
            i = 0;
            do {
                unsigned char b2 = *(unsigned char *)((long)elems + 0x12);
                unsigned long u8 = b2; unsigned int u7 = b2;
                unsigned long word;
                if (u7 == 1) {
                fetch:
                    if (u8 <= i) { word = 0; }
                    else {
                        unsigned long *p = elems;
                        if (1 < u7 - 1) p = (unsigned long*)*elems;
                        word = p[i];
                    }
                } else {
                    if (u7 == 5) { u8 = (unsigned long)*(unsigned int *)(elems + 1); goto fetch; }
                    if (b2 == 2) { u8 = 2; goto fetch; }
                    word = 0;
                }
                cL4_arg_decode(result, (unsigned long)tcb, word, depth_u + 1);
                if (*result != 0) return;
                i = i + 1;
            } while (n != i);
        } else if (eb == 5) {
            n = (unsigned long)*(unsigned int *)(elems + 1);
            if (*(unsigned int *)(elems + 1) != 0) goto arr_loop;
        } else if (eb == 2) {
            n = 2; goto arr_loop;
        }
    }
tail:
    byte = 0x47;
    cL4_mr_emit_byte((unsigned long)tcb + 0x428, &byte, tcb[0x42a]);
    cL4_out_commit((unsigned long)tcb, (long*)&desc);
success:
    *result = 0; result[2] = 0; result[3] = 0; result[4] = 0;
}
