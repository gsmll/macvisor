...
void cL4_grow_alloc(unsigned long buf, unsigned long elems);              /* FUN_003d216c */
...
l2:
    ...
    if (*result == 0) {
        ...
        cL4_msg_three(result, tcb, (long*)a2, depth + 1);
        if (*result == 0) {
...
l1:
    ...
    a1 = cur[1];
l2:
    cL4_msg_three(result, tcb, (long*)a1, depth + 1);
    if (*result == 0) {
...
void cL4_ipc_emit_mxa(int *result, unsigned long tcb, unsigned long *msg, unsigned long depth)
{
    ...
    a0 = *cur;
l1:
    cL4_msg_one(result, tcb, (long*)a0, depth + 2);
    if (*result != 0) return;
    cur = msg;
    if (*(char *)((long)msg + 0x12) != 2) {
        if ((*(char *)((long)msg + 0x12) != 5) || (*(unsigned int *)(msg + 1) < 2)) { a1 = 0; goto l2; }
    ...
    a1 = cur[1];
l2:
    cL4_ipc_emit_list(result, tcb, a1, depth + 1);
    if (*result == 0) {
...
l1:
    ...
    a1 = cur[1];
l2:
    cL4_msg_one(result, tcb, (long*)a1, depth + 2);
    if (*result == 0) {
 ...
 * after each element except the first (first is marked "y" when it is also
 * the last). Loops `count` times per the descriptor shape (kind 1 = 1, kind 2
 * = 2, kind 5 = array count). Confidence: medium. */
...
{
    unsigned long *cur = msg; unsigned long *pl;
...
void cL4_ipc_emit_r(unsigned long *result, unsigned long tcb, unsigned long *msg, unsigned long depth)
{
    ...
    if (*(char *)((long)msg + 0x12) != 2) {
        if ((*(char *)((long)msg + 0x12) != 5) || ((int)msg[1] != 2)) {
            result[0] = 1; result[1] = msg; result[2] = 0x470;
            return;
        }
        cur = (unsigned long*)*msg;
    }
    ...
    if ((-1 < pdata[3]) && (pdata[3] == 0)) {
        result[0] = 1; result[1] = msg; result[2] = 0x479;
        return;
    }
    pval = (int)pdata[3];
    if (pval == 1) {
    ...
    if (*(char *)((long)msg + 0x12) == 5) msg = (unsigned long*)*msg;
    cL4_mr_emit_num(tcb, (long)**(unsigned long **)((long)msg + 8));
    cL4_msg_emit_pair(tcb, (long*)pdata[3], (unsigned long)&DAT_005be7c0, 0x7a);
done:
...
/* 003d08c4 @ 0x3d08c4   (est. cL4_msg_dispatch_full)
 * Ghidra: void FUN_003d08c4(int *result, undefined8 *tcb, long *msg, undefined8 depth)
 * Full message dispatch. When the message kind is composite (cL4_msg_kind
 * non-zero), it re-dereferences the header via cL4_msg_header and recurses on
 * the nested descriptor. For known single-word kinds (0x19/0x3f/0xe7/0xf6/0xb1/
 * 0xf5) it emits the corresponding tag ("C"/"O"/"V"/"a"/"XY") and, for kind
 * 0xf5, invokes a per-object callback. An over-deep (>0x400) or unknown kind
 * sets error code 0x223/0x24d. Confidence: medium. */
void cL4_msg_dispatch_full(int *result, unsigned long *tcb, unsigned long *msg, unsigned long depth)
{
    ...
    if (!cL4_msg_kind(msg)) {
        ...
        if (type < 0xe7) {
            ...
            {
                unsigned long v = ((unsigned long (**)(void))tcb[0x434])(tcb[0x435], 0, *msg);
    ...
    tmp[0] = 0; tmp[1] = 0;
    if (cL4_fmt_begin((unsigned long)tcb, (unsigned long)msg, (unsigned char(*)[16])tmp, 0)) goto done;
    if (cL4_msg_header(&b, msg, *tcb) != 0) {
        /* propagate header result */
        result[1] = tmp[1]; *result = tmp[0]; result[2] = tmp[2];
        return;
    }
    cL4_msg_dispatch_full(result, tcb, (unsigned long*)tmp[2], depth + 1);
    if (*result != 0) return;
    b = 0x79;
    cL4_msg_dispatch_pkt(result, tcb, msg, &b, depth + 1, 0);
    if (*result != 0) return;
    if ((*(char *)((long)msg + 0x12) != 5) || ((int)msg[1] != 3)) goto fin;
    {
        unsigned long *slot = *(unsigned long **)(*msg + 0x10);
        unsigned char k; unsigned long n, i;
        k = *(unsigned char *)((long)slot + 0x12);
        n = k;
        if (k == 1) {
            i = 0;
            do {
                unsigned long word;
                k = *(unsigned char *)((long)slot + 0x12);
                if (k == 1) { if (n <= i) word = 0; else { unsigned long *p = slot; if (1 < k - 1) p = (unsigned long*)*slot; word = p[i]; } }
                else if (k == 5) { n = *(unsigned int *)(slot + 1); word = ((unsigned long*)*slot)[i]; }
                else if (k == 2) { n = 2; word = slot[i]; }
                else word = 0;
                          ...
                          unsigned long depth, int flag)
{
    ...
    if (type < 0x22) {
        if ((t9 != 0x12) && (0x11 < type)) {
            ...
            if (*(unsigned char *)((long)payload + 0x12) - 1 < 2) {
                v = *payload;
            } else if ((*(unsigned char *)((long)payload + 0x12) == 5) && (*(int *)(payload + 1) != 0)) {
            ...
            *byte = '_';
            {
                ...
                if (*(char *)((long)msg + 0x12) == 2) { w = p[1]; }
                else if ((*(char *)((long)msg + 0x12) == 5) && (1 < *(unsigned int *)(msg + 1))) { p = (unsigned long*)*msg; w = p[1]; }
                else w = 0;
                cL4_msg_args(result, tcb, p, depth + 1);
            }
            goto fin2;
        }
        if (t9 - 0xd < 6) {
            ...
            if (*(short *)(payload + 2) == 0xf5) {
                unsigned long (*cb)(void) = *(unsigned long (**)(void))(tcb + 0x21a0);
                unsigned long *r = (unsigned long*)cb(*(unsigned long *)(tcb + 0x21a8), 0, *payload);
...
group_big:
    ...
    }
    {
        ...
        }
        {
            ...
            cL4_msg_dispatch_pkt(result, tcb, w, byte, depth + 1);
        }
        if (*result != 0) return;
        goto fin2;
    }
...
void cL4_grow_buffer_ensure(unsigned long alloc_ctx, unsigned long *buf, unsigned int *cap, unsigned long need)
{
    ...
    if ((*buf + n * 0x10 == *(long *)(alloc_ctx + 8)) &&
        ...
        *(unsigned long *)(alloc_ctx + 8) = newsize;
        ...
        if (need < 5) need = 4;
        {
            long nb = cL4_grow_alloc(alloc_ctx, need + n);
...
void cL4_grow_alloc(unsigned long buf, unsigned long elems)
{
    ...
    if ((cap != 0) && (map[4] != 0)) {
        ...
        if (node != 0) {
            ...
            do {
                if (node == 0) return 0;
                if (node[1] == hv) {
                    if ((cL4_map_keyeq(node + 2, key) & 1) != 0) return (long)node;
                } else {
    ...
    if (cap != 0) {
        ...
        if (head != 0) {
            ...
            for (; it != 0; it = (unsigned long*)*it) {
                ...
                if (v == hv) {
                    if ((cL4_map_keyeq(it + 2, key) & 1) != 0) return it;
                } else {
                    if ((cap & mask) == 0) v = v & mask;
                    else if (cap <= v) { unsigned long d = 0; if (cap != 0) d = v / cap; v = v - d * cap; }
                    if (v != idx) break;
...
void cL4_demangle_node(unsigned long out, unsigned long *node, unsigned long depth)
{
    ...
    cL4_out_putc(out, 10);
    {
        ...
        for (; cur != end; cur = cur + 1) cL4_demangle_node(out, *cur, depth + 1);
    }
}
