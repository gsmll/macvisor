...
extern sk_u128_t rt_001afe4c() __attribute__((noreturn)); /* FUN_001afe4c (shared cL4 runtime library, out of scope) */
...
sk_u128_t sk_ubp_count2(void);                           /* 002a4a2c */
word_t sk_ubp_owner(void);                               /* 002a4a7c */
word_t sk_ubp_owner2(void);                              /* 002a4a80 */
void sk_ubp_ensure(long a);                              /* 002a4ab4 */
long sk_ubp_alloc_raw(word_t a,word_t b,word_t c,uint d);/* 002a4b90 */
...
void sk_ubp_store(word_t a,word_t b,word_t c);           /* 002a5130 */
...
word_t sk_ubp_append_slice(word_t a,word_t b,word_t c,word_t d); /* 002a55c0 */
word_t sk_ubp_append_slice2(word_t a,word_t b,word_t c,word_t d,word_t e,word_t f); /* 002a58b8 */
word_t sk_ubp_append_many(word_t a,word_t b,long c);     /* 002a5bf0 */
sk_u128_t sk_ubp_append_pair(word_t a,word_t b,word_t c,word_t d); /* 002a5e9c */
word_t sk_ubp_append_char(word_t a,word_t b);            /* 002a6080 */
void sk_ubp_format(void *a,void *b,void *c,void *d,void *e); /* 002a6238 */
void sk_ubp_grow(void);                                  /* 002a66f0 */
sk_u128_t sk_ubp_insert(word_t a,word_t b,word_t c,word_t d,word_t e,word_t f); /* 002a67a0 */
sk_u128_t sk_ubp_insert2(word_t a,word_t b,word_t c,word_t d,word_t e,word_t f); /* 002a6900 */
sk_u128_t sk_ubp_insert3(word_t a,word_t b,word_t c,word_t d); /* 002a6a18 */
sk_u128_t sk_ubp_insert4(word_t a,word_t b,word_t c,word_t d,word_t e); /* 002a6ad8 */
sk_u128_t sk_ubp_insert5(word_t a,word_t b,word_t c,word_t d,word_t e); /* 002a6be4 */
sk_u128_t sk_ubp_insert6(word_t a,word_t b,word_t c,word_t d,word_t e,word_t f); /* 002a6d08 */
sk_u128_t sk_ubp_insert7(word_t a,word_t b,word_t c,word_t d); /* 002a6e3c */
void sk_ubp_grow2(void);                                 /* 002a6efc */
void sk_ubp_merge(word_t a,word_t b);                    /* 002a70d8 */

/* Helper for the "SoftwareBreakpoint(n,pc)" artifacts the decompiler recovers as
 * noreturn in-kernel traps (takes a code pointer from the software-breakpoint
 * handler). These are unrecoverable in this slice and are nulled for analysis: the
 * macro yields the scalar 0 so it may appear both as a statement and inside the
 * conditional guards the decompiler wraps around the trap. */
#define SW_BREAK(n,pc) 0
/* Ghidra LZCOUNT (count leading zeros); used by UTF-8 decode. */
...
word_t sk_swift_str_next_scalar(void)
{
    ...
    if (0) {
        ...
        if (((uint)u >> 1 & 1) == 0) {
            if ((0xffff < u) && (u = rt_0034cbb0(u >> 0x10).lo, 0 != 0)) {
                rt_00350624();
                u = rt_002c6aac().lo;
                return u;
            }
...
{
    sk_swift_str_prelude(); a[0] = (void *)0;
}
...
{
    *(sk_u128_t *)a = sk_swift_str_next_break();
}
...
word_t sk_swift_str_eq_sub(void *a, void *b, sk_fnptr c, sk_fnptr d, sk_fnptr e, sk_fnptr f)
{
    ...
    if (0) {
        if (d == 0) {
            ...
            sk_swift_str_compare_eq(); return 0;
        }
        if (d == 0) {
            u = rt_0008e500(0,0);
            r = u.lo & 0xffffffffffff;
...
word_t sk_swift_str_compare_sub(void *a, long b, void *c, long d, char e, uint f)
{
    ...
    sk_swift_str_compare_bytes(); return 0;
}
...
range_panic:
        ...
        rt_001e4cbc();
        rt_00351790();
        l = sk_swift_memcmp_limited((void *)(uintptr_t)t.lo,(long)t.hi,0,0);
        if (l == 0) {
            rt_00353238(0 ^ 1,0);
            return;
        }
...
{
    sk_ubp_alloc_raw((word_t)a,(word_t)b,(word_t)b,(uint)(uintptr_t)c);
}
...
void sk_swift_str_finalize(void *a, long b, uint c)
{
    ...
    if (b == 0) {
        ...
        sk_ubp_alloc_raw((word_t)a,(word_t)b,(word_t)b,c & 1);
        rt_00355c10();
    }
}
...
void sk_ubp_read(void **a, void **b)
{
    ...
    if ((u & 1) == 0) {
        ...
        rt_000e15d8();
        sk_swift_str_copyin(0,0); a[0] = (void *)0; a[1] = (void *)0;
        a[1] = (void *)0;
    }
}
...
bool sk_ubp_is_ascii(void *a, void *b, void *c, sk_fnptr d)
{
    ...
    if (((u < 0x21) && ((0x100003e01U >> (u & 0x3f) & 1) != 0)) ||
       ((*d)((unsigned char *)(buf + 0x20),c), (p = (char *)0) == (char *)0x0)) {
        r = false;
    } else {
        r = *p == '\0';
    }
 ...
 * Confidence: medium */
sk_u128_t sk_ubp_count2(void)
{
    word_t obj;
    if (((0 & (0 ^ 0xffffffffffffffff)) >> 0x3d & 1) != 0) {
        obj = 0 & 0xfffffffffffffff;
        if (rt_003a261c(obj).lo != 0) {
            return (sk_u128_t){(*(word_t *)(obj + 0x10) & 0xffffffffffff) - 1,0};
        }
    }
    return (sk_u128_t){rt_00068e14().lo,0};
}
...
void sk_ubp_ensure(long a)
{
    ...
    u = 0;
    if ((u >> 0x3c & 1) != 0) {
        sk_ubp_destroy((void *)(uintptr_t)a);
        return;
    }
    if ((u >> 0x3d & 1) == 0) {
        rt_003510b8();
 ...
 * Confidence: medium */
long sk_ubp_alloc_raw(word_t a, word_t b, word_t c, uint d)
{
    long l;
    word_t u;
    sk_u128_t t;
 ...
 * Confidence: medium
 * Notes: re-transcribed against decompile 2026-08-12 (was degraded: registers/branches
 *  replaced with 0, dropped FUN_002a45bc + pcVar12 capture). unaff_x19-x23 modeled as
 *  callee-saved register locals. extraout_x1/x8, in_CY, and the args to sk_ubp_finish
 *  (002a507c) are Swift-pair-ABI register artifacts not recoverable from the decompile
 *  and are noted inline. */
void sk_ubp_append(void)
{
    sk_fnptr pc12;             /* address of sk_ubp_append2 (FUN_002a4c98), captured in x? */
    word_t u, u2, u3, u4, u7, u9, u10, u11, u1, x1r, x3r;
    word_t reg19, reg21, reg22, reg23;   /* unaff_x19/x21/x22/x23 (callee-saved inputs) */
    word_t *reg20;             /* unaff_x20: pointer to the two buffer words */
    long l8;
    sk_u128_t t13, t14;
    int i6;
    bool b5;                   /* uVar5 */
    uint8_t cy;                /* in_CY carry flag artifact */

    sk_ubp_fatal();                              /* FUN_002a45bc */
    pc12 = (sk_fnptr)sk_ubp_append2;                /* FUN_002a4c98 */
    rt_0035193c();
    rt_00353274();
    rt_00351a38();
    x1r = 0;  /* extraout_x1: x1 return of FUN_00351a38 (Swift pair ABI), not recoverable */
    if ((x1r >> 0x3c & 1) == 0) {
        u11 = reg22 - reg23;                     /* unaff_x22 - unaff_x23 */
        if (reg22 < reg23) {                     /* SBORROW8(unaff_x22,unaff_x23) */
            pc12 = (sk_fnptr)SoftwareBreakpoint(1,0x2a4f1c);
            pc12();
        }
    } else {
        rt_00350974(reg23 << 0x10 | 1, reg22 << 0x10 | 1);
        rt_00354404();
        rt_002ab8ac();
        u9 = rt_00356290().lo;
        u2 = 0;                                  /* extraout_x8_00 */
    ...
    u1 = u10 + u11;
    b5 = u1 == 0;
    if (u10 > (word_t)~u11) {                    /* SCARRY8(uVar10,uVar11): add overflow */
        pc12 = (sk_fnptr)SoftwareBreakpoint(1,0x2a4f18);
        pc12();
    }
    if ((((u7 & (u3 ^ 0xffffffffffffffff)) >> 0x3d & 1) == 0) ||
        (i6 = (int)rt_003a261c(u3 & 0xfffffffffffffff).lo, i6 == 0)) {
        u4 = 0xf < u1;
        b5 = u1 == 0x10;
        if ((long)u1 < 0x10) goto LAB_002a4da8;
    } else {
        rt_00350a34();
        sk_ubp_endptr(u7, u3);                   /* FUN_002a49fc */
        u7 = rt_00351db4().lo;
        if (b5) {
            rt_00351088();
            goto LAB_002a4f44;
        ...
        if ((long)u1 < 0x10) {
            if ((u3 >> 0x3d & 1) == 0) {
                u4 = u11 <= u7;
                b5 = u7 == u11;
                if ((long)u11 <= (long)u7) goto LAB_002a4d1c;
            }
LAB_002a4da8:
            rt_00350a34();
            sk_ubp_init_short2(reg23 << 0x10 | 1, reg22 << 0x10 | 1); /* FUN_002a4f80 */
            rt_003534e0();
            rt_00350974(reg23 << 0x10 | 1, reg22 << 0x10 | 1);
            rt_00354404();
            rt_002ab8ac();
            ...
            rt_0036b270();
            rt_0034ba38(u2);
            sk_ubp_move(0,0,0,0);                /* FUN_002a3e64 */
            rt_00353918();
            rt_003a25d4();
            rt_003504ac();
            sk_ubp_init_short2(0,0);             /* FUN_002a4f80 */
            rt_0006b674();
            rt_003a25d4(0);                      /* extraout_x1_00 */
            rt_00350738();
            rt_0034b89c();
            if (b5) {
LAB_002a4f44:
...
LAB_002a4d1c:
    ...
    l8 = rt_00351db4().lo;
    if ((((u7 & (u3 ^ 0xffffffffffffffff)) >> 0x3d & 1) == 0) ||
        (t13 = rt_003a261c(u3 & 0xfffffffffffffff), (uint32_t)t13.lo == 0)) {
        u7 = u1;
        if (b5 || l8 < (long)u11) goto LAB_002a4e44;
LAB_002a4e64:
        sk_ubp_ensure(u7);                       /* FUN_002a4ab4 */
    } else if (b5 || l8 < (long)u11) {
LAB_002a4e44:
        rt_00351354();
        t14 = sk_ubp_capacity(0,0);              /* FUN_002a49d4 (reg args opaque) */
        u7 = 0;
        if ((t14.hi & 0xff) != 1) {              /* auVar14._8_4_ & 0xff */
            u7 = t14.lo << 1;
        }
        if ((long)u7 <= (long)u1) {
            u7 = u1;
    ...
    if ((reg21 >> 0x3c & 1) != 0) {              /* unaff_x21 */
        rt_003507e0();
        rt_00351e3c();
        rt_00351774();
        sk_ubp_append_ci(0,0,0,0);               /* FUN_002a50ac */
        return;
    }
    if ((reg21 >> 0x3d & 1) == 0) {
        if ((reg19 >> 0x3c & 1) == 0) {          /* unaff_x19 */
            rt_003507e0();
            rt_002a9ba8();
            rt_00351790();
        } else {
            x3r = reg19 & 0xffffffffffff;        /* in_x3 */
        }
        rt_003504c4();
        rt_003520dc();
        t13 = rt_0028e8c0();
        if ((-1 < t13.lo) && (t13.hi <= (long)x3r)) {
            rt_00348e48();
            t13 = rt_001e4cbc();
            rt_00351774(t13.lo, t13.hi, reg19 >> 0x3f, pc12);
            sk_ubp_finish();                     /* FUN_002a507c */
            return;
        }
    } else {
        u11 = reg21 >> 0x38 & 0xf;
        x3r = reg19;                             /* in_stack_00000010 */
        rt_003504c4(t13.lo, t13.hi, &x3r);
        rt_003520dc();
        t13 = rt_0028e8c0();
        if ((-1 < t13.lo) && (t13.hi <= (long)u11)) {
            rt_00348e48();
            t13 = rt_001e4cbc();
            sk_ubp_finish();                     /* FUN_002a507c(lo,hi,reg21>>0x3e&1): args dropped */
LAB_002a4e2c:
            rt_00351774(pc12);
            return;
        }
...
{
    sk_ubp_store((word_t)*(void **)(0 + 8),0,0);
    rt_00358d1c();
}
 ...
 * Confidence: low */
void sk_ubp_store(word_t a, word_t b, word_t c)
{
    sk_u128_t t;
...
{
    sk_ubp_append_slice((word_t)c,(word_t)d,a >> 0x10,b >> 0x10);
}
 ...
 * Confidence: low */
word_t sk_ubp_append_slice2(word_t a, word_t b, word_t c, word_t d, word_t e, word_t f)
{
    word_t u, v;
    long l;
    sk_u128_t t, r;
    ...
    if (0) {
        ...
        *(word_t *)(0 + 8) = t.hi;
        ...
        u = c;
        v = d;
        r = sk_ubp_insert4(a,b,(word_t)&u,0,0);
    }
    return v;
}
 ...
 * result. Returns 0.
 * Confidence: low */
word_t sk_ubp_append_many(word_t a, word_t b, long c)
{
    word_t u, v, n;
    long l;
    sk_u128_t t, r;
    ...
    v = 0;
    if (0) {
        return sk_ubp_insert5(a,b,(word_t)c,(word_t)&LAB_002a6794,0).lo;
    }
    t.lo = 0;
    t.hi = 0xe000000000000000;
    r = sk_ubp_capacity(u,v);
 ...
 * Confidence: low */
sk_u128_t sk_ubp_append_pair(word_t a, word_t b, word_t c, word_t d)
{
    word_t u, v, n;
    long l;
    sk_u128_t t, r;
    ...
    if (0) {
        rt_0036b270(d);
        t = sk_ubp_insert6(a,b,c,d,(word_t)&LAB_002a6794,0);
        rt_003a25d4(d);
        return t;
    }
    t = sk_ubp_capacity(u,v);
 ...
 * Confidence: low */
word_t sk_ubp_append_char(word_t a, word_t b)
{
    word_t u, v, n;
    long l;
    sk_u128_t t, r;
    ...
    v = 0;
    if (0) {
        return sk_ubp_insert7(a,b,(word_t)&LAB_002a6794,0).lo;
    }
    t = sk_ubp_capacity(u,v);
    l = t.lo;
    if (((t.lo & 0xff) != 1 && 0xf < l) &&
 ...
 * Confidence: low */
sk_u128_t sk_ubp_insert(word_t a, word_t b, word_t c, word_t d, word_t e, word_t f)
{
    sk_u128_t t, r;
    long l, n;
    word_t u;
 ...
 * Confidence: low */
sk_u128_t sk_ubp_insert2(word_t a, word_t b, word_t c, word_t d, word_t e, word_t f)
{
    sk_u128_t r;
    long l, n;
    word_t u;
 ...
 * Confidence: low */
sk_u128_t sk_ubp_insert3(word_t a, word_t b, word_t c, word_t d)
{
    sk_u128_t r;
    long l;
    word_t u;
 ...
 * Confidence: low */
sk_u128_t sk_ubp_insert4(word_t a, word_t b, word_t c, word_t d, word_t e)
{
    sk_u128_t r;
    long l, n;
    word_t u;
 ...
 * Confidence: low */
sk_u128_t sk_ubp_insert5(word_t a, word_t b, word_t c, word_t d, word_t e)
{
    sk_u128_t r;
    long l, n;
    word_t u;
 ...
 * Confidence: low */
sk_u128_t sk_ubp_insert6(word_t a, word_t b, word_t c, word_t d, word_t e, word_t f)
{
    sk_u128_t r;
    long l, n;
    word_t u;
 ...
 * Confidence: low */
sk_u128_t sk_ubp_insert7(word_t a, word_t b, word_t c, word_t d)
{
    sk_u128_t r;
    long l;
    word_t u;
...
void sk_ubp_merge(word_t a, word_t b)
{
    ...
    if (l != 0) {
        a = a + 0x28;
        do {
            word_t cur0 = *(word_t *)0;        /* current buffer word0 (unaff_x20[0]) */
            word_t cur1 = *(word_t *)(0 + 8);  /* current buffer word1 */
            u = *(word_t *)(a - 8);            /* element word0 */
            v = *(word_t *)(uintptr_t)a;       /* element word1 */
            n = cur0 & 0xffffffffffff;
            if ((cur1 & 0x2000000000000000) != 0) {
                n = cur1 >> 0x38 & 0xf;
            }
            if ((n == 0) && (((cur0 & (cur1 ^ 0xffffffffffffffff)) >> 0x3d & 1) == 0)) {
                rt_0036b270(v);
                rt_003a25d4(cur1);
                *(word_t *)0 = u;
                *(word_t *)(0 + 8) = v;
