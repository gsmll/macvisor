...
void sk_re_diag_init(word_t, ...);	/* FUN_0042f584 @ 0x0042f584 */
...
sw128_t sk_re_peek_scalar(void);	/* FUN_0042fea0 @ 0x0042fea0 */
...
word_t sk_re_parse_mode_option(word_t a, word_t b, word_t c, word_t d);	/* FUN_0043519c @ 0x0043519c */
        ...
        if ((rec[1] & 1) == 0) {
                ...
                while (true) {
                        ...
                        if (buf[0] == '\0') {
                                ...
                                if ((g & 1) == 0) { sw_462710(*(word_t*)(base + 0x10)); sw_6b3f4(0); base = (long)sw_45636c(0); }
...
sw128_t sk_re_peek_scalar(void)
{
        sw128_t sc = sw_b4594(0);
        word_t *rec = (word_t*)__builtin_frame_address(0);
        word_t pos = rec[2];
...
sw128_t sk_re_str_slice(word_t a, word_t b, word_t *p)
{
        ...
        if (s1 == 0) { out.lo = 0; out.hi = 0; }
                ...
                sk_re_parse_scalar_adv(0, 0);
                out.lo = sw_2a3e64(r.lo, r.hi, s0, s1);
                sw_3a25d4(s1);
...
byte sk_re_scalar_to_utf8(word_t a, word_t b, word_t c)
{
        ...
        byte r;
        sw_24917c((word_t)&r, (word_t)__builtin_frame_address(0));
        sw_36b118(t);
        return r;
}
...
void sk_re_diag_inv_scalar(word_t a, word_t b, word_t c, word_t d)
{
        ...
        if (!(v & 0x100)) {
                if (!(((uint)(v >> 0x10) & 0xffff) < 0x11 && (int)v - 0xe000 < -0x800)) {
                        ...
                        sw_863bc(0);
                        sw_1ebfb0((word_t)&v, 0x10, 1, 0x6776f0, 0);
                        sw_2acbb8(0);
                        sw_3a25d4(0);
                        sk_re_diag_rec(0);
                        sw_461430(0);
 ...
 * current scalar, chooses the base (8/10/16) from the caller record, decodes
 * the digit run (decimal string form with +/-/bare accumulation, sign-prefixed
 * form carrying the radix in x23 bits 56-59, or a strtoul-like helper for
 * non-decimal radices) with overflow trapping, and writes the 5-word result
 * record (value, span, x21, x22, kind) through extraout_x8.
 * Confidence: low (heavy register-forwarding in the digit loops).
 * Notes: traps 0x431204/0x431208/0x43120c/0x431210; dispatch table 0x6860e8;
 *   SCARRY8/SBORROW8 overflow checks in the accumulation loops are opaque. */
sw128_t sk_re_parse_number(void)
{
        byte *rec = (byte*)__builtin_frame_address(0);
        sw128_t s = sw_356f9c(0);
        word_t u12 = *(word_t*)((s.lo & 0xff) * 8 + 0x6860e8);   /* dispatch table */
        bool bv7 = true;
        sw_68e14(0);
        word_t u13 = 0;
        sk_re_parse_suffix2();
        if (0) {   /* extraout_x1 == 0 (opaque gate): degenerate cleanup path */
                u12 = 0;
                sw_463f40(0);
                word_t u15 = 0;   /* extraout_x8_00 (opaque) */
                goto done;
        }
        sw_3562d4(0);
        long radix;
        ...
        /* span: from unaff_w24:0x40c4 (bits) & 0xffffffffffff, or x23 radix nibble */
        word_t u11 = 0 & 0xffffffffffff;   /* CONCAT44(0x40c4, unaff_w24) (opaque) */
        word_t u14 = 0 & 0xf;              /* unaff_x23 >> 0x38 & 0xf (opaque) */
        word_t u15 = u11;
        if (0 /* unaff_x23 & 0x2000000000000000 */) u15 = u14;
        word_t x21 = u12;                  /* unaff_x21 */
        word_t x22 = u13;                  /* unaff_x22 */
        sw128_t au21 = {0,0};
        if (u15 == 0) goto err;
        if (0 == 0 /* unaff_x23 >> 0x3c & 1 == 0 */) {
                if (1 /* unaff_x23 >> 0x3d & 1 != 0 */) {
                        /* sign-prefixed form; radix carried in x23 bits 56-59 */
                        if (u14 == 0) SW_FATAL(0x431208);
                        word_t c = 0 & 0xff;   /* unaff_w24 & 0xff (opaque) */
                        bool u6;
                        if (c == 0x2b) {        /* '+' */
                                u6 = u14 != 0;
                                if (u14 - 1 == 0) { au21 = (sw128_t){0, 1}; goto chk; }
                                sw_462c7c(u14 - 1);
                                do {
                                        sw_46507c(0);
                                        /* digit/hex accumulation (opaque SCARRY8/SBORROW8) */
                                        if (1) goto negone;   /* opaque overflow term */
                                        u6 = 0 != 0;          /* extraout_x8_04 */
                                } while (0 != 1);             /* extraout_x8_04 != 1 */
                        } else {
                                u6 = 0x2c < c;
                                if (c == 0x2d) {        /* '-' */
                                        u6 = u14 != 0;
                        ...
                        au21.lo = u15; au21.hi = 0;
                        goto chk;
                }
                /* decimal string form: (ptr=lo, len=hi) from aux helpers */
                sw128_t au22;
                if (0 /* unaff_000040c4 >> 0x1c & 1 == 0 */) {
                        sw_350518(0);
                        au22 = sw_2a9ba8(0);
                } else {
                        au22.hi = u11;
                        au22.lo = (0 & 0xfffffffffffffff) + 0x20;   /* (x23 & mask)+0x20 */
                }
                long len = au22.hi;
                byte *p = (byte*)au22.lo;
                if (len < 1) SW_FATAL(0x431204);
                if (*p == '+') {
                        if ((len != 1) && (p == (byte*)0x0)) SW_FATAL(0x431210);
                        if (len != 1) {
                                long r = sw_462c7c(0);
                                if (r == 0) goto err2;
                                do {
                                        sw_46507c(0);
                                        if (1) goto err;   /* opaque overflow term */
                                } while (0 != 1);
                                goto commit2;
                        }
                        goto err;
                ...
                if (*p == '-') {
                        if ((len != 1) && (p == (byte*)0x0)) SW_FATAL(0x43120c);
                        if (len == 1) goto err;
                        long r = sw_462c7c(0);
                        if (r == 0) goto err2;
                        do {
                ...
                {
                        /* bare digit accumulation (radix lVar9, opaque register flow) */
                        long acc = 0;
                        while (p != (byte*)0x0) {
                                byte b = *p;
                                long hi = 0, hi0 = 0;
                                /* bVar3 digit class: -0x30/-0x37/-0x57 offsets */
                                char off;
                                if (b >= 0x30 && b < (radix == 10 ? 0x3a : radix + 0x37))
                                        off = -0x30;
                                else if (b >= 0x41 && b < radix + 0x37) off = -0x37;
                                else if (b >= 0x61 && b < radix + 0x57) off = -0x57;
                                else goto err;
                                acc = acc * radix + (long)(b + off);
                                len = len - 1;
                                p = p + 1;
                                if (len == 0) break;
                        }
                ...
                /* non-decimal radix via the strtoul-like helper 0xb077c */
                sw_36b270(0);
                au21 = sw_350518(0);
                sw128_t r2 = sw_350518(0);
                au21 = (sw128_t){ sw_0b077c(r2.lo, r2.hi, radix), 0 };
                sw_3a25d4(0);
chk:
                if ((au21.hi & 0xff) == 1) goto err;
        }
        u12 = au21.lo;
        sw_3a25d4(0);
        bv7 = false;
...
commit2:
        ...
        goto done;
negone:
        au21 = (sw128_t){0, 1};   /* ZEXT816(1) << 0x40 */
        goto chk;
err2:
        au21 = (sw128_t){0, 0};   /* ZEXT816(0) */
        u12 = au21.lo;
        sw_3a25d4(0);
        bv7 = false;
        u15 = au21.hi & 0xff;
...
err:
        ...
        /* output record write (extraout_x8): value, span, x21, x22, kind */
        ((word_t*)rec)[0] = u12;
        ((word_t*)rec)[1] = u15;
        ((word_t*)rec)[2] = x21;
        ((word_t*)rec)[3] = x22;
        ((byte*)rec)[0x20] = (byte)bv7;
        sw_356f80(s.hi);
        return (sw128_t){u12, u15};
}
...
sw128_t sk_re_parse_ref_number(long expected)
{
        ...
        sk_re_diag_inv_scalar(out.lo, out.hi, rec[2], v);
        sw_3a25d4(0);
        return out;
}
...
is_u:                   sw_463590(0x75);
                ...
                sw128_t v = sk_re_parse_ref_number(c);
                sk_re_diag_rec(0);
        }
        sw_1a1c8(0);
        sw_117cc4(0, 0, 0x99);
...
void sk_re_parse_group_body(void)
{
        ...
        word_t start = *(word_t*)(__builtin_frame_address(0) + 0x10);
        sw_4657d0(0);
        word_t m = sk_re_append_match2(0, 0);
        word_t pos = start;
        if ((m & 1) == 0) {
                sw_462d0c(0);
                word_t ok;
...
{
        word_t out[2];
        sk_re_parse_comment(&out, 0, 0, 0, 0);
        if (out[1] == 0) {
...
void sk_re_parse_option_block(word_t a, word_t b, word_t c, word_t *out)
{
        ...
        sk_re_peek_char();
        if (0 == 0) {
                sw128_t s = sk_re_peek_scalar2();
                /* collect mode options */
                word_t mode = sk_re_parse_mode_option(0, 0, 0, 0);
                while (mode != 0x14) {
                        /* append (option, span) record */
                        sw_45904c(0);
                        word_t m = sk_re_parse_mode_option(0, 0, 0, 0);
                        if (m == 0x14) break;
                }
        } else {
                sw_465920(0);
...
void sk_re_parse_option_end(void)
{
        ...
        if (0 != 0) {
                ...
                sw_1ee018(0);
                sk_re_diag_complete();
                sk_re_parse_option_block(0, 0, 0, (word_t*)&rec);
        } else {
                sw_4658fc(0);
        }
        /* write result record */
...
{
        word_t rec[10];
        sw_463f80((word_t)&rec[0]);
        sk_re_record_select((sw128_t){0,0},(sw128_t){0,0},(sw128_t){0,0},(sw128_t){0,0},(sw128_t){0,0});
        if (rec[9] == -1) {
                word_t pos = *(word_t*)(__builtin_frame_address(0) + 0x10);
                sw_3524a4((word_t)&rec[2]);
                sw_4653fc(0);
                word_t hi = rec[3];
                word_t lo = rec[2];
...
void sk_re_record_join(sw128_t p2, sw128_t p3, sw128_t p4, sw128_t p5, sw128_t p6)
{
        ...
        sw_464adc(0);
        sw_4614cc(0, 0);
        sw128_t h = sk_re_parse_group_header(0, 0, 0);
                                        ...
                                        if ((sk_re_diag_emit_str() & 1) == 0) {
                                                ...
                                                if ((sk_re_diag_emit_str() & 1) == 0) {
                                                        ...
                                                        if ((sk_re_diag_emit_str() & 1) == 0) {
                                                                ...
                                                                if ((sk_re_diag_emit_str() & 1) == 0) {
                                                                        /* option block */
                                                                        sk_re_parse_option_block(0, 0, 0, 0);
                                                                        sk_re_parse_backref(0, 0);
                                                                        goto done;
                                                                }
                                                        }
                                                }
                ...
                if ((m & 1) == 0) {
                        ...
                        if (0 == 0) {
                                ...
                                if (0 == 0) {
                                        ...
                                        sk_re_parse_backref_or_scalar();
                                        if ((0 & 0xff) == 0) {
                                                sw_464b20(0);
                                                sw_4614cc(0, 0);
                                                sw_4636e8(0);
                                                sk_re_parse_capture_name(0, 0, 0, 0);
...
void sk_re_parse_concat(void)
{
        ...
        if ((m & 1) == 0) {
                ...
                if ((a & 1) == 0) {
                        sw_84180(r.lo, r.hi, 0);   /* uVar4 reset to 0 before LAB_00439414 */
                        sw_84234(0);
                        return;
                }
                sw_4652c4(0);
...
void sk_re_parse_expr(void)
{
        ...
        sw128_t r;
        do {
                sk_re_parse_comment(buf, 0, 0, 0, 0);
                r.lo = buf[0]; r.hi = buf[1];
                word_t pos = *(word_t*)(__builtin_frame_address(0) + 0x10);
                if (r.hi == 0) {
                        if ((buf[1] & 1) == 0) {
                                sw_465144(0);
        ...
        sk_re_parse_anchor();
        sk_re_diag_merge((word_t)rec, 0, 0, 0);
        sw_45904c(0);
        sw_b4390(0);
}
