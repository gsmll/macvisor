...
/* A demangle-tree "Node": a 16-bit kind code plus a payload. The tag byte
 * (node->tag, at +0x12) selects the payload layout:
 *   0 = empty;  1 = one child (word0);  2 = two children (word0,word1);
 *   3 = {payload ptr, count} in word0/word1;  4 = payload ptr only;
 *   5 = array of child Node pointers beginning at word0. */
typedef struct dem_node {
    unsigned long  p0;      /* +0x00 */
    unsigned long  p1;      /* +0x08 */
...
/* Demangler state (param_1 of the parse functions). */
typedef struct dem_state {
    unsigned long  pad0[7];        /* +0x00 .. +0x37 */
    const char    *src;            /* +0x38 source base */
    unsigned long  src_end;        /* +0x40 */
    unsigned long  pos;            /* +0x48 position */
    unsigned char  flag50;         /* +0x50 */
    unsigned char  flag51;         /* +0x51 */
    unsigned long  pad52;          /* +0x52 */
    dem_node_t   **stack;          /* +0x58 node stack */
    unsigned int   stack_count;    /* +0x60 */
    unsigned int   pad64;
    dem_node_t   **stack2;         /* +0x68 second stack */
    unsigned int   stack2_count;   /* +0x70 */
    unsigned int   pad74;
    unsigned long  locals[26][2];  /* +0x78 local sub-identifier table */
    unsigned int   local_count;    /* +0x218 */
    unsigned int   pad21c;
    unsigned long  collect[2];     /* +0x220 small-vector collector */
    unsigned long  cb;             /* +0x238 external callback */
} dem_state_t;

/* In-slice prototypes. */
static unsigned long cL4_dem_resolve(unsigned long *op, unsigned long *x);
static void cL4_dem_node_reserve(dem_state_t *st, unsigned long n);

/* ------------------------------------------------------------------ *
 * Function bodies (address order)
 * ------------------------------------------------------------------ */

/* FUN_0039fcc4 @ 0x0039fcc4   (est. cL4_dem_msg_kind)
 * Ghidra: ulong FUN_0039fcc4(ulong *param_1)
 * Returns the low 32 bits of the message/operation word *param_1, but only
 * when the word is < 0x800 (a valid demangle operation token); otherwise 0.
 * Confidence: medium
 */
unsigned long cL4_dem_msg_kind(unsigned long *word)
{
    unsigned long v = 0;
    if (*word < 0x800) {
        v = *word & 0xffffffff;
...
/* FUN_0039fcd8 @ 0x0039fcd8   (est. cL4_dem_node_kind_get)
 * Ghidra: ulong FUN_0039fcd8(undefined8 param_1, ulong *param_2, undefined8 param_3)
 * Resolves the demangle operation described by param_2 (or its derived
 * operand) and dispatches on its kind token to fetch the matching node kind
 * value from one of the runtime tables (0x67c1c8/0x67c0d0/0x67c230/0x67b0a8),
 * i.e. translates a raw operation code into a specific demangle node kind.
 * Out-of-range tokens hit a fail-closed SoftwareBreakpoint(1) trap.
 * Confidence: low
 */
unsigned long cL4_dem_node_kind_get(unsigned long op, unsigned long *word,
                                    unsigned long x)
{
    unsigned long r, kind;
    long t;
    unsigned long *base, *cand;
    long lv;

    cL4_w16_t rv = cL4_dem_resolve((unsigned long*)x, (unsigned long*)op);
    ...
    if (kind < 0x203) {
        if (kind - 0x201U < 2) {
            r = 0x67c1c8;
            return cL4_dem_node_kind_get2(&r, &r);
        }
        if (kind != 0) {
            if (kind != 0x200) return 0;
            r = 0x67c0d0;
            return cL4_dem_node_kind_get3(&r, &r);
        }
    } else if (kind < 0x301) {
        if (kind != 0x203) {
            if (kind != 0x300) return 0;
            if ((base != (unsigned long*)0x67b0a8) ||
                (0xfffffffffffff800 < **(long **)*cand - 0x800U))
                return 0;
            if (word == 0) {
                t = cL4_dem_ctx_cur();
...
L_use_230:
    r = 0x67c230;
    return cL4_dem_node_kind_get4(cand, &r);
}
