 ...
 * Ghidra: void FUN_000402b4(long param_1,long param_2)
 * Fetches the capability chain covering a given 16K slot from the VAS spanmap,
 * records it back into the vspace span cursor, and validates the returned
 * capability type. A nonzero type byte inside the L4 error-code range aborts;
 * otherwise the caller's error slot is set to 0x102 if it was empty.
 * Confidence: medium
 * Notes: string "spanmap_get_cap had unexpected i.." (0x5b43ff); SoftwareBreakpoint
 *   trampoline at 0x403b0; error table bound check against DAT_0064cb40.
 */
void sk_spanmap_get_cap(unsigned long vas, unsigned long slot)
{
    unsigned long span, err;
    unsigned long res[6];
    unsigned long i;

    for (i = 0; i < 6; i++) res[i] = 0;
    span = *(unsigned long *)(vas + 0x38);
    if (span != 0 &&
        (span + 0xd0 < span || *(unsigned long *)(vas + 0x40) < span + 0xd0 ||
         span < *(unsigned long *)(vas + 0x48)))
        sk_vas_abort(0x5b43ff);

    sk_spanmap_get(res, *(unsigned long *)(*(unsigned long *)(vas + 0x30) + 0x78),
        (unsigned long)((*(unsigned long *)(*(unsigned long *)(*(unsigned long *)(vas + 0x20) + 8) + 0x18) +
               slot * 0x4000 - *(unsigned long *)(span + 0x28)) >> 0xe) + 1);
    *(unsigned long *)(*(unsigned long *)(vas + 0x30) + 0x78) = res[4];
    if (res[0] == 0) {
        err = res[2] & 0xff;
        if (err != 0) {
            if (err < 9)
        ...
        if (*(unsigned long *)(*(unsigned long *)(vas + 0x28) + 8) == 0)
            *(unsigned long *)(*(unsigned long *)(vas + 0x28) + 8 + 0x18) = 0x102;
    }
}

/*--------------------------------------------------------------------*/
/* FUN_000403b0 @ 0x000403b0   (est. sk_spanmap_cap_op)
 * Ghidra: void FUN_000403b0(long param_1,ulong param_2,long param_3,undefined8 param_4)
 * Dispatches a capability operation on the VAS spanmap: op 0 maps the slot's
 * 16K region through the spanmap and faults in backing; op 0x107 succeeds as a
 * no-op; other small ops invoke the span object's handler. Nonzero results are
 * translated to an L4 error word; unexpected results abort.
 * Confidence: medium
 * Notes: FUN_00042abc spanmap lookup; FUN_0003fcc8 va translate; string 0x5b44af.
 */
void sk_spanmap_cap_op(unsigned long vas, unsigned long op, long slot, unsigned long arg)
{
    unsigned long span, res, base;
    unsigned long errbuf[4];
    long ret;

    base = *(unsigned long *)(*(unsigned long *)(*(unsigned long *)(vas + 0x20) + 8) + 0x18) + slot * 0x4000;
    if (op == 0) {
        span = *(unsigned long *)(vas + 0x30);
        if (span == 0 ||
            (span <= span + 0xd0 && span + 0xd0 <= *(unsigned long *)(vas + 0x38) &&
             *(unsigned long *)(vas + 0x40) <= span)) {
            res = sk_spanmap_lookup(span, 1, base);
            span = *(unsigned long *)(vas + 0x30);
            if (span == 0 ||
                (span <= span + 0xd0 && span + 0xd0 <= *(unsigned long *)(vas + 0x38) &&
                 *(unsigned long *)(vas + 0x40) <= span)) {
                ret = sk_spanmap_resolve(*(unsigned long *)(vas + 0x48), base, span, arg, res, 0, 1);
                if (ret != 0) {
                    span = *(unsigned long *)(vas + 0x30);
                    if (*(unsigned long *)(vas + 0x38) >= span + 0xd0 && span >= *(unsigned long *)(vas + 0x40))
                        sk_x_004b3538(span, arg, vas, ret);
                }
            }
        }
        return;
    ...
    if ((op & 0xfd) == 0) {
        span = *(unsigned long *)(vas + 0x30);
        if (*(unsigned long *)(vas + 0x38) < span + 0xd0 || span < *(unsigned long *)(vas + 0x40))
            sk_vas_abort(0x5b44af);
        res = (*(unsigned long (**)(unsigned long,unsigned long))(*(unsigned long *)(span + 0xb8) + 0x10))
                        (*(unsigned long *)(span + 0xb0), arg);
        if ((res & 0xff) == 0) {
            ret = *(unsigned long *)(*(unsigned long *)(vas + 0x28) + 8);
            if (*(unsigned long *)(ret + 0x18) == 0)
                *(unsigned long *)(ret + 0x18) = op;
            return;
        }
        sk_vas_err(errbuf, res);
    }
    sk_vas_err(errbuf, op);
    if (*(unsigned long *)(vas + 0x30) + 0xd0 <= *(unsigned long *)(vas + 0x38) &&
        *(unsigned long *)(vas + 0x40) <= *(unsigned long *)(vas + 0x30)) {
        sk_vas_abort(0x5b44af);
    }
}

/*--------------------------------------------------------------------*/
/* FUN_000405d0 @ 0x000405d0   (est. sk_spanmap_next_used)
 * Ghidra: long FUN_000405d0(undefined8 param_1,long param_2)
 * Walks the spanmap chain looking for the next "used" entry (offset 0x20 != 0),
 * bounding the walk by the 64 GiB span sentinel. Returns the entry or 0.
 * Confidence: medium
 */
unsigned long sk_spanmap_next_used(unsigned long ctx, long ent)
{
    while (1) {
        if (ent == 0) return 0;
        if (*(int *)(ent + 0x20) != 0) break;
        if (*(unsigned long *)(ent + 0x10) + *(unsigned long *)(ent + 8) == 0x1000000000) return 0;
        ent = (long)sk_spanmap_next(ctx);
    }
    return ent;
}

/*--------------------------------------------------------------------*/
/* FUN_00040630 @ 0x00040630   (est. sk_spanmap_query)
 * Ghidra: undefined1 [16] FUN_00040630(long param_1,ulong param_2,undefined8 *param_3)
 * Resolves a virtual address to its backing span/attribute, returning
 * {error, value}. Validates the VA range, locks the span, reads the spanmap,
 * and either returns the direct (error,value) pair or calls the span object's
 * resolve method. Aborts on invalid vspace geometry or lock failure.
 * Confidence: medium
 * Notes: strings 0x5b4b49 / 0x5aed68; cl4_result-shaped return.
 */
cl4_result_t sk_spanmap_query(unsigned long vas, unsigned long va, unsigned long outp)
{
    cl4_result_t r;
    unsigned long span, lv, res[6];
    unsigned long i, err;

    r.lo = 0; r.hi = 0;
    for (i = 0; i < 6; i++) res[i] = 0;
    if (*(unsigned long *)(vas + 0x10) <= (va & 0xffffffffffffc000) - *(unsigned long *)(vas + 8))
        sk_vas_abort(0x5b4b49);
    lv = *(unsigned long *)(vas + 0x40);
    *(unsigned long *)outp = 0xffffffffffffffff;
    if ((*(unsigned char *)(vas + 0x21) >> 4 & 1) == 0) {
        span = sk_spanmap_lock(*(unsigned long *)(vas + 0x50));
        err = sk_mtx_lock(vas + 0x80);
        if (err) sk_vas_abort(0x5aed68);
        sk_spanmap_get_v2(res, *(unsigned long *)(vas + 0x78),
            (unsigned long)((va - *(unsigned long *)(span + 0x28) >> 0xe)) + 1);
        *(unsigned long *)(vas + 0x78) = res[4];
        if (res[0] == 0) {
            err = sk_mtx_unlock(vas + 0x80);
            if (err) sk_vas_abort(0x5aed68);
            if ((res[2] & 0xff) == 0) { r.lo = 0x9370002; r.hi = 0; goto out; }
            lv = (**(unsigned long (**)(unsigned long,unsigned long,unsigned long))(*(unsigned long *)(span + 0xb8) + 0x18))
                        (*(unsigned long *)(span + 0xb0), res[0], outp);
            err = sk_mtx_unlock(vas + 0x80);
            if (err) sk_vas_abort(0x5aed68);
            if (lv != 0) { r.lo = 0x93e0001; r.hi = 0; goto out; }
            r.lo = 0; r.hi = 0; goto out;
        } else {
            lv = (**(unsigned long (**)(unsigned long,unsigned long,unsigned long))(*(unsigned long *)(span + 0xb8) + 0x18))
                        (*(unsigned long *)(span + 0xb0), res[0], outp);
            err = sk_mtx_unlock(vas + 0x80);
        ...
        r.lo = 0x9260001;
        if (*(unsigned long (**)(unsigned long,unsigned long,unsigned long))(vas + 0x28) != 0) {
            r.lo = (**(unsigned long (**)(unsigned long,unsigned long,unsigned long))(vas + 0x28))
                        (*(unsigned long *)(vas + 0x38), (va & 0xffffffffffffc000) - lv, outp);
            if ((r.lo & 1) == 0) { r.lo = 0x9280001; r.hi = 0; goto out; }
            r.lo = 0; r.hi = 0; goto out;
        }
    }
    r.hi = 0;
...
/*--------------------------------------------------------------------*/
/* FUN_00040870 @ 0x00040870   (est. sk_spanmap_rb_insert)
 * Ghidra: void FUN_00040870(ulong *param_1,ulong param_2)
 * Inserts a node into the spanmap interval tree rooted at *param_1, recomputing
 * each ancestor's max-extent field (0x68) and the balance field (0x24), then
 * re-links via the tree rebalance helper. A fresh root directly installs the
 * node and computes its extent from the child extents.
 * Confidence: medium
 */
void sk_spanmap_rb_insert(unsigned long rootp, unsigned long node)
{
    unsigned long cur, l, r, max, nb, bl, br;

    cur = *(unsigned long *)rootp;
    if (cur == 0) {
        *(unsigned long *)rootp = node;
        if (*(int *)(node + 0x20) == 0) nb = *(unsigned long *)(node + 0x10);
        else nb = 0;
        l = 0; if (*(unsigned long *)(node + 0x58)) l = *(unsigned long *)(*(unsigned long *)(node + 0x58) + 0x68);
        r = 0; if (*(unsigned long *)(node + 0x60)) r = *(unsigned long *)(*(unsigned long *)(node + 0x60) + 0x68);
        max = l <= r ? r : l;
        *(unsigned long *)(node + 0x68) = nb <= max ? max : nb;
        return;
    }
    if (*(unsigned long *)(node + 8) < *(unsigned long *)(cur + 8)) {
        sk_spanmap_rb_insert(cur + 0x58, node);
        l = *(unsigned long *)(cur + 0x58);
    } else {
        sk_spanmap_rb_insert(cur + 0x60, node);
        l = *(unsigned long *)(cur + 0x60);
    }
    if (*(unsigned char *)(cur + 0x24) <= *(unsigned char *)(l + 0x24))
        *(unsigned char *)(cur + 0x24) = *(unsigned char *)(l + 0x24) + 1;
    bl = 0; if (*(unsigned long *)(cur + 0x58)) bl = *(unsigned char *)(*(unsigned long *)(cur + 0x58) + 0x24) + 1;
    br = 0; if (*(unsigned long *)(cur + 0x60)) br = ~(unsigned int)*(unsigned char *)(*(unsigned long *)(cur + 0x60) + 0x24);
    sk_rbtree_rebalance(rootp, br + bl);
}

/*--------------------------------------------------------------------*/
/* FUN_00040980 @ 0x00040980   (est. sk_spanmap_startfault_cow)
 * Ghidra: void FUN_00040980(long param_1,long param_2,uint param_3,long param_4,undefined1 *param_5)
 * Copy-on-write fault resolution on a VAS span. Locks the span, reads the
 * spanmap to locate the faulting slot, and promotes the mapping to a writable
 * COW page by allocating a fresh page and copying the shared content. Records
 * the fault result (kind, address, size) into the caller's out-struct.
 * Confidence: medium
 * Notes: string "startfault_impl_cow" (0x5b5920); span lock strings 0x5b2e2d;
 *   stat counters at DAT_006ad304..; COW via sk_vas_span_op (0x3fa94).
 */
void sk_spanmap_startfault_cow(unsigned long ff, long va, unsigned int flags, unsigned long vas, unsigned long out)
{
    unsigned long span, res[6], lv, ret;
    unsigned long i, err;

    sk_stat_cow++;
    for (i = 0; i < 6; i++) res[i] = 0;
    if ((*(unsigned char *)(ff + 0xa0) & 1) != 0)
        sk_vas_abort(0x5b5920);
    if (*(unsigned long *)(ff + 0x90) == 0 || *(unsigned long *)(ff + 0x98) == 0)
        sk_vas_abort(0x5b5920);
    span = sk_spanmap_lock(*(unsigned long *)(vas + 0x50));
    err = sk_mtx_lock(vas + 0x80);
    if (err) sk_vas_abort(0x5aed68);
    sk_spanmap_get(res, *(unsigned long *)(vas + 0x78),
        (unsigned long)((va - *(unsigned long *)(span + 0x28)) >> 0xe) + 1);
    lv = res[0];
    *(unsigned long *)(vas + 0x78) = res[4];
    err = sk_mtx_unlock(vas + 0x80);
    if (err) sk_vas_abort(0x5aed68);
    if (lv != 0) {
        if (1 < flags) { sk_stat_cow2++; ret = 0xe360001; goto done; }
        CallSupervisor(0);
        if (lv == 0) { sk_stat_cow3++; ret = 0; goto done; }
        /* COW: allocate and copy, then record the fault. */
        ret = sk_vas_span_op(ff + 0x58, ff + 0x60, *(unsigned long *)(ff + 0x98), 0);
        if ((ret & 0xff) == 0) {
            if (*(unsigned long *)(ff + 0x88) < 0x4000) sk_vas_abort(0x5b5920);
            *(unsigned char *)out = 1;
            *(unsigned int *)(out + 1) = 0;
            *(unsigned int *)(out + 4) = 0;
            *(unsigned long *)(out + 8) = 0x4000;
            *(unsigned long *)(out + 0x10) = va;
            *(unsigned long *)(out + 0x18) = *(unsigned long *)(ff + 0x60);
...
/*--------------------------------------------------------------------*/
/* FUN_00040f80 @ 0x00040f80   (est. sk_spanmap_fault_cont)
 * Ghidra: void FUN_00040f80(void)
 * Continuation helper invoked after a spanmap fault to resume fault resolution
 * on the span's owner. Small trampoline into the COW path's error-decorator.
 * Confidence: low
 */
void sk_spanmap_fault_cont(void) { }

/*--------------------------------------------------------------------*/
/* FUN_0004108c @ 0x0004108c   (est. sk_vas_err_stack16)
 * Ghidra: void FUN_0004108c(void)
 * Translates an error code into the standard L4 error word at &stack+0x10.
...
/*--------------------------------------------------------------------*/
/* FUN_000410ac @ 0x000410ac   (est. sk_vas_err_stack18)
 * Ghidra: void FUN_000410ac(void)
 * Translates an error code into the standard L4 error word at &stack+0x18.
 * Confidence: medium (trivial wrapper).
 */
...
/*--------------------------------------------------------------------*/
/* FUN_000410cc @ 0x000410cc   (est. sk_vas_noop)
 * Ghidra: void FUN_000410cc(void)
 * No-op.
...
/*--------------------------------------------------------------------*/
/* FUN_000410e4 @ 0x000410e4   (est. sk_vas_abort_5b324e)
 * Ghidra: void FUN_000410e4(undefined8 param_1)
 * Aborts with the VAS error at 0x5b324e.
 * Confidence: medium
 */
void sk_vas_abort_5b324e(unsigned long ret)
{
    (void)ret;
    sk_vas_abort(0x5b324e);
}

/*--------------------------------------------------------------------*/
/* FUN_000410f8 @ 0x000410f8   (est. sk_vas_noop_f8)
 * Ghidra: void FUN_000410f8(void)
 * No-op.
 * Confidence: high (trivial).
 */
...
/*--------------------------------------------------------------------*/
/* FUN_00041110 @ 0x00041110   (est. sk_vas_noop_110)
 * Ghidra: void FUN_00041110(void)
 * No-op.
 * Confidence: high (trivial).
 */
...
/*--------------------------------------------------------------------*/
/* FUN_0004112c @ 0x0004112c   (est. sk_vas_err_stack)
 * Ghidra: void FUN_0004112c(void)
 * Translates an error into the L4 error word via FUN_0003f980.
 * Confidence: medium (trivial wrapper).
 */
...
/*--------------------------------------------------------------------*/
/* FUN_00041138 @ 0x00041138   (est. sk_vas_noop_138)
 * Ghidra: void FUN_00041138(void)
 * No-op.
 * Confidence: high (trivial).
 */
...
/*--------------------------------------------------------------------*/
/* FUN_0004114c @ 0x0004114c   (est. sk_vas_noop_14c)
 * Ghidra: void FUN_0004114c(void)
 * No-op.
 * Confidence: high (trivial).
 */
...
/*--------------------------------------------------------------------*/
/* FUN_00041160 @ 0x00041160   (est. sk_vas_abort_l4err)
 * Ghidra: void FUN_00041160(void)
 * Aborts with the generic "Unexpected L4 Error" at 0x5b010f.
 * Confidence: medium
 */
...
/*--------------------------------------------------------------------*/
/* FUN_00041170 @ 0x00041170   (est. sk_vas_abort_5b5fc3)
 * Ghidra: void FUN_00041170(undefined8 param_1)
 * Aborts with the VAS error at 0x5b5fc3.
 * Confidence: medium
 */
void sk_vas_abort_5b5fc3(unsigned long ret)
{
    (void)ret;
    sk_vas_abort(0x5b5fc3);
}

/*--------------------------------------------------------------------*/
/* FUN_00041184 @ 0x00041184   (est. sk_vas_abort_5b56c0)
 * Ghidra: void FUN_00041184(undefined8 param_1)
 * Aborts with the VAS error at 0x5b56c0.
 * Confidence: medium
 */
void sk_vas_abort_5b56c0(unsigned long ret)
{
    (void)ret;
    sk_vas_abort(0x5b56c0);
}

/*--------------------------------------------------------------------*/
/* FUN_00041198 @ 0x00041198   (est. sk_vas_abort_5b562b)
 * Ghidra: void FUN_00041198(undefined8 param_1)
 * Aborts with the VAS error at 0x5b562b.
 * Confidence: medium
 */
void sk_vas_abort_5b562b(unsigned long ret)
{
    (void)ret;
    sk_vas_abort(0x5b562b);
}

/*--------------------------------------------------------------------*/
/* FUN_000411ac @ 0x000411ac   (est. sk_vas_abort_5b4c32)
 * Ghidra: void FUN_000411ac(undefined8 param_1)
 * Aborts with the VAS error at 0x5b4c32.
 * Confidence: medium
 */
void sk_vas_abort_5b4c32(unsigned long ret)
{
    (void)ret;
    sk_vas_abort(0x5b4c32);
}

/*--------------------------------------------------------------------*/
/* FUN_000411c0 @ 0x000411c0   (est. sk_vas_noop_1c0)
 * Ghidra: void FUN_000411c0(void)
 * No-op.
 * Confidence: high (trivial).
 */
...
/*--------------------------------------------------------------------*/
/* FUN_000411dc @ 0x000411dc   (est. sk_vas_query_bounds)
 * Ghidra: void FUN_000411dc(undefined8 param_1,ulong *param_2)
 * Queries the VAS address-range geometry: returns the (base,limit) pair of the
 * selected address-space region, computing the default region size from the
 * platform config when type 1. Validates the region type and range ordering.
 * Confidence: medium
 * Notes: FUN_00041308/0004137c/000413c4 geometry helpers; abort 0x5b6272.
 */
void sk_vas_query_bounds(unsigned long type, unsigned long outp)
{
    unsigned long lo, hi, a, b;

    if (type == 1 && (sk_vas_cfg_init() & 1) == 0) {
        sk_x_004b3ac8();
    } else {
        if (outp == 0) { lo = 0; hi = 0xffffffffffffffff; }
        else { lo = *(unsigned long *)outp; hi = *(unsigned long *)(outp + 8); }
        a = sk_vas_region_count(type);
        b = sk_vas_region_size(type);
        lo = lo <= a ? lo : a;
        hi = hi <= b ? hi : b;
        if (hi <= lo || b <= a)
            sk_vas_abort(0x5b6272);
        if (type - 1 < 2 || type == 0) return;
    }
    sk_vas_abort(0x5b6272);
}

/*--------------------------------------------------------------------*/
/* FUN_00041308 @ 0x00041308   (est. sk_vas_cfg_init)
 * Ghidra: undefined1 FUN_00041308(void)
 * Lazily initializes the per-CPU VAS configuration globals (DAT_006af2b8 flag,
 * DAT_006af2b9/ba/bb geometry bytes) from the CPU config and returns the
 * "root level" byte from the cfg. Idempotent; guarded by the init flag.
 * Confidence: medium
 * Notes: FUN_0005baac cfg alloc, FUN_0005530c/60/44 geometry fields.
 */
unsigned long sk_vas_cfg_init(void)
{
    ...
    if ((sk_cfg_flag & 1) == 0) {
        cfg = sk_cpu_cfg(4, 0xc);
        if (cfg == 0) {
            sk_cfg_geo_a = sk_cfg_a();
            sk_cfg_geo_b = sk_cfg_b();
            sk_cfg_geo_c = sk_cfg_c();
            sk_cfg_install(&sk_vas_root);
        }
        sk_cfg_flag = 1;
    }
    return *(unsigned char *)sk_vas_cfg();
}

/*--------------------------------------------------------------------*/
/* FUN_0004137c @ 0x0004137c   (est. sk_vas_region_count)
 * Ghidra: undefined8 FUN_0004137c(uint param_1)
 * Returns the number of level regions for a given VAS geometry type (table at
 * DAT_004bc520). Aborts for out-of-range types.
 * Confidence: medium
 */
unsigned long sk_vas_region_count(unsigned int type)
{
    if (type < 3)
        return *(unsigned long *)(sk_pmm_pgtable + (unsigned long)type * 8);
    sk_vas_abort(0x5b6400);
    return 0;
}

/*--------------------------------------------------------------------*/
/* FUN_000413c4 @ 0x000413c4   (est. sk_vas_region_size)
 * Ghidra: long FUN_000413c4(int param_1)
 * Returns the size of a VAS region for the given geometry type: 64 GiB for
 * type 0, 4 TiB for type 2, and for type 1 the platform's 1<<(rootlevel-1).
 * Aborts on unknown types.
 * Confidence: medium
 */
long sk_vas_region_size(int type)
{
    if (type == 0) return 0x1000000000;
    if (type == 2) return 0x40000000000;
    if (type != 1) sk_vas_abort(0x5b6400);
    return 1L << ((unsigned long)*(unsigned char *)(sk_vas_cfg() + 1) & 0x3f);
}

/*--------------------------------------------------------------------*/
/* FUN_00041434 @ 0x00041434   (est. sk_vas_layout_init)
 * Ghidra: long FUN_00041434(long param_1,int param_2,ulong param_3,long param_4,
 *   undefined8 param_5, undefined8 param_6)
 * Initializes a VAS geometry descriptor: records the geometry type at 0x98,
 * base at 0x28, limit at 0x30, and span method table at 0xb0/0xb8. Computes the
 * number of level regions from the type and fills each region's (base-mask,
 * count) triple at 0x38, returning the total mapped size.
 * Confidence: medium
 */
long sk_vas_layout_init(unsigned long d, int type, unsigned long base, long limit,
                        unsigned long mtab0, unsigned long mtab)
{
    unsigned long lv, nreg, idx, mask, cnt, acc;
    long total;

    *(unsigned char *)(d + 0x98) = (char)type;
    *(unsigned long *)(d + 0x28) = base;
    *(unsigned long *)(d + 0x30) = limit;
    *(unsigned long *)(d + 0xb0) = mtab0;
    *(unsigned long *)(d + 0xb8) = mtab;

    if (type == 0) nreg = 2;
    else if (type == 2) nreg = 3;
    else {
        if (type != 1) sk_vas_abort(0x5b6400);
        lv = (unsigned long)*(unsigned char *)(sk_vas_cfg() + 1) - 0xe;
        nreg = lv / 0xb;
        if (lv % 0xb != 0) nreg++;
    }
    total = 0; idx = 0;
    do {
        mask = -1L << ((idx * 0xb + 0x16) & 0x3f) & base;
        cnt = (limit + ~(-0x4000L << (idx * 0xb & 0x3f))) - mask >> (idx * 0xb + 0xe & 0x3f);
        *(unsigned long *)(d + 0x38 + idx * 0x18 + 8) = mask;
        *(unsigned long *)(d + 0x38 + idx * 0x18 + 0x10) = cnt;
        acc = (cnt & 0xff) ? 0x100 : 0;
        total += (acc + (cnt & 0x3ffffffffffff00)) * 0x40;
        idx = (idx & 0xff) + 1;
    } while ((unsigned char)idx <= nreg);
    return total;
}

/*--------------------------------------------------------------------*/
/* FUN_0004158c @ 0x0004158c   (est. sk_vas_init_root)
 * Ghidra: void FUN_0004158c(undefined8 param_1,long param_2,long param_3)
 * One-time initialization of the kernel's root VAS: installs the spanmap method
 * table, lays out the geometry, scans all pmm page-table entries to remap the
 * root span's level regions, sets up the six top-level page-table slots, and
 * registers the fault-handler callbacks. Marks the vspace initialized (0x100).
 * Confidence: medium
 * Notes: strings 0x5b636f; DAT_006af298/DAT_0064c450 span table; walk via
 *   sk_pmm_bootmem (0x53470); SoftwareBreakpoint trampolines.
 */
void sk_vas_init_root(unsigned long p, long a, long b)
{
    unsigned long cfg, span, u, va, i, lvl, n;
    unsigned long walk;
    cl4_result_t r;

    (void)p; (void)a; (void)b;
    cfg = sk_cpu_cfg(4, 0xb);
    if (cfg == 0) {
        sk_vas_root = 0;   /* DAT_0064c450 span table */
        sk_cfg_install(sk_err_codes2);
    }
    cfg = sk_cpu_get(0x6af2b0, 4, 0xb);
    if ((*(unsigned char *)(cfg + 0x100) & 1) != 0) return;

    span = *(unsigned long *)(cfg + 0xf8);
    if (span <= span + 0xd0) {
        sk_vas_query_bounds(0, 0);
        r = sk_cap_type_table();
        sk_vas_layout_init(span, 0, r.lo, (long)r.hi, r.lo, r.hi);
        if (*(unsigned long *)(span + 0x48) >> 0xe == 0)
            sk_vas_abort(0x5b636f);
        walk = sk_pmm_bootmem();
        span = sk_cpu_get(0x6af2b0, 4, 0xb);
        u = *(unsigned long *)(span + 0xf8);
        if (u <= u + 0xd0) {
            /* determine level count from geometry type byte at +0x98 */
            lvl = *(unsigned char *)(u + 0x98);
            if (lvl == 0) lvl = 2;
            else if (lvl == 2) lvl = 3;
            else {
                lvl = sk_vas_cfg();
                n = ((unsigned long)*(unsigned char *)(lvl + 1) - 0xe) / 0xb;
                if (((unsigned long)*(unsigned char *)(lvl + 1) - 0xe) % 0xb) n++;
                lvl = n;
            }
            /* scan pmm tables, remap root level regions (condensed) */
            while (walk != 0xffffffffffffffff) {
                if (*(unsigned long *)walk != 0)
                    sk_vas_abort(0x5b636f);
                walk = *(unsigned long *)(walk + 8);
            }
            /* set up 6 top-level PT slots at +0x60..+0x77 */
            *(unsigned long *)(span + 0x78) = 6;
            *(unsigned char *)(span + 0x101) = 6;
            va = 0;
            for (i = 0; i < 6; i++) {
                *(unsigned long *)(span + 0x60 + i * 8) = va;
                va -= 0x40;
            }
...
/*--------------------------------------------------------------------*/
/* FUN_00041d9c @ 0x00041d9c   (est. sk_vas_teardown_free)
 * Ghidra: void FUN_00041d9c(void)
 * Tears down a VAS: frees the per-level page-table entries referenced from the
 * geometry descriptor (DAT_006af2b0, offsets 0x30..0x78), frees the final-level
 * pages, and releases the descriptor's backing spans. Runs under the pmm lock.
 * Confidence: medium
 * Notes: aborts into SoftwareBreakpoint trampoline at 0x41f50 on bounds fail.
 */
void sk_vas_teardown_free(void)
{
    unsigned long cfg, span, v, n, i, lvl;
    unsigned char t;

    cfg = sk_cpu_get(0x6af2b0, 4, 0xb);
    for (i = 0x30; i != 0x60; i += 8) sk_pmm_free(*(unsigned long *)(cfg + i));
    for (i = 0x60; i != 0x78; i += 8) sk_pmm_free(*(unsigned long *)(cfg + i));
    span = sk_cpu_get(0x6af2b0, 4, 0xb);
    v = *(unsigned long *)(span + 0xf8);
    if (v <= v + 0xd0) {
        t = *(unsigned char *)(v + 0x98);
        if (t == 0) lvl = 1;
        else if (t == 2) lvl = 2;
        else {
        ...
        for (i = 0; i <= lvl; i++) {
            n = *(unsigned long *)(v + 0x48 + i * 0x18);
            if (n > 0xff) return;
            while (n != 0) { sk_pmm_free(*(unsigned long *)(v + 0x38 + i * 0x18)); n--; }
        }
    }
    sk_vas_abort(0x5b6400);
}
...
/*--------------------------------------------------------------------*/
/* FUN_00041f8c @ 0x00041f8c   (est. sk_vas_cfg)
 * Ghidra: void FUN_00041f8c(undefined8 ...)
 * Returns the shared VAS platform configuration descriptor, initializing it on
 * first use from the current geometry globals. The descriptor describes root
 * level, page-table level counts, and the span method table.
 * Confidence: medium
 * Notes: DAT_006af2b8 init flag; DAT_006af2c0 config object.
 */
unsigned long sk_vas_cfg(void)
{
    return sk_cpu_get(0x6af2c0, 4, 0xc);
}

/*--------------------------------------------------------------------*/
/* FUN_00041fbc @ 0x00041fbc   (est. sk_vas_shadow_setup)
 * Ghidra: void FUN_00041fbc(long param_1,ulong param_2,long param_3,long param_4,
 *   undefined8 param_5, undefined8 param_6, undefined8 param_7, undefined8 param_8,
 *   undefined4 param_9)
 * Sets up the shadow (second-stage) address space for a VAS: lays out the level
 * regions, maps the top-level page-table region, and, for ASID-type 1, allocates
 * and installs the shadow root via the pmm allocator with per-level page sizes.
 * On failure it decorates the error with the AppleInternal path and aborts.
 * Confidence: medium
 * Notes: strings "vas_core_shadow_space_setup" (0x5b6466), "__s__zu___pmm_alloc"
 *   (0x5b6752), "__s__zu___xrt__plat_Dart_SetRoot" (0x5b6670).
 */
void sk_vas_shadow_setup(unsigned long vas, unsigned long type, long phys, long size,
                         unsigned long m0, unsigned long m1, unsigned long a, unsigned long b,
                         unsigned int flags)
{
    unsigned long lv, u, count;
    unsigned long ret;
    unsigned char t;

    lv = sk_vas_layout_init(vas, (int)type, m0, size, a, b);
    t = *(unsigned char *)(vas + 0x98);
    *(unsigned long *)(vas + 0x18) = phys;
    *(unsigned long *)(vas + 0x20) = size;
    if (t == 0) count = 2;
    else if (t == 2) count = 3;
    else {
        if (t != 1) sk_vas_abort(0x5b6400);
        u = sk_vas_cfg();
        count = ((unsigned long)*(unsigned char *)(u + 1) - 0xe) / 0xb;
        if (((unsigned long)*(unsigned char *)(u + 1) - 0xe) % 0xb) count++;
    }
    *(unsigned int *)(vas + 0xc0) = flags;
    if (sk_mtx_init(vas + 0xa0)) sk_vas_abort(0x5aed68);
    /* map root region via span method table (condensed) */
    ret = sk_x_004b3bc0();
    *(unsigned long *)(vas + 0x10) = 0;
    sk_vas_abort(0x5b6466);
}

/*--------------------------------------------------------------------*/
/* FUN_00042640 @ 0x00042640   (est. sk_l4_err_string)
 * Ghidra: void FUN_00042640(undefined8 *param_1,byte param_2)
 * Fills a 32-byte buffer with the string name of an L4 error code (0..9), or
 * the unknown code marker for out-of-range codes. Strings at DAT_004bc3c0+.
...
void sk_l4_err_string(unsigned long out, unsigned long code)
{
    ...
    if (code > 9) {
        *(unsigned long *)(out + 8) = *(unsigned long *)0x4bc508;
        *(unsigned long *)out = *(unsigned long *)0x4bc500;
        *(unsigned long *)(out + 0x18) = *(unsigned long *)0x4bc518;
        *(unsigned long *)(out + 0x10) = *(unsigned long *)0x4bc510;
    ...
    *(unsigned long *)(out + 0x18) = *(unsigned long *)((unsigned long)p + 0x18);
    *(unsigned long *)(out + 0x10) = *(unsigned long *)((unsigned long)p + 0x10);
    *(unsigned long *)(out + 8) = *(unsigned long *)((unsigned long)p + 8);
}

/*--------------------------------------------------------------------*/
/* FUN_00042754 @ 0x00042754   (est. sk_vas_pt_alloc_flags)
 * Ghidra: uint FUN_00042754(long param_1,ulong param_2)
 * Returns the per-level page-table allocation flags for a VAS at a given level:
 * type 2 uses level 0, type 1 reads the config's level flag, and type 0 selects
 * from a bit table by level (0xc0d0e04 >> level*8). Aborts on bad type.
 * Confidence: medium
 */
unsigned int sk_vas_pt_alloc_flags(unsigned long vas, unsigned long level)
{
    ...
    if (t == 2) return 0;
    if (t == 1) {
        if (level - 1 > 2) sk_vas_abort(0x5b6e04);
        cfg = sk_vas_cfg();
        return (unsigned int)*(unsigned char *)(cfg + 2);
    }
    if (t != 0) sk_vas_abort(0x5b6e4b);
    return (0xc0d0e04 >> (((unsigned int)level & 3) << 3)) & 0xff;
}

/*--------------------------------------------------------------------*/
/* FUN_00042808 @ 0x00042808   (est. sk_vas_populate_level)
 * Ghidra: ulong FUN_00042808(undefined8 *param_1,undefined8 param_2,long param_3)
 * Populates a VAS page-table level: for each region entry it either issues a
 * bulk unmap (via the span object's bulk method) or walks the level's PTEs
 * clearing each entry, then tears down the region's page tables. Also validates
 * and walks the root region. Returns an L4 error code.
 * Confidence: medium
 * Notes: string "populate_level_nonself" (0x5b7444); spanmap walk 0x42abc.
 */
unsigned long sk_vas_populate_level(unsigned long vas, unsigned long va, long size)
{
    char t;
    unsigned long nreg, i, j, ent, res, lv, base;
    unsigned long eb[4];

    t = (char)*(unsigned long *)(vas + 0x98);
    if (t == 0) nreg = 2;
    else if (t == 2) nreg = 3;
    else {
        if (t != 1) sk_vas_abort(0x5b6400);
        lv = sk_vas_cfg();
        nreg = ((unsigned long)*(unsigned char *)(lv + 1) - 0xe) / 0xb;
        if (((unsigned long)*(unsigned char *)(lv + 1) - 0xe) % 0xb) nreg++;
        if (nreg == 0) goto no_regions;
    }
    i = 0;
    do {
        ent = *(unsigned long *)(vas + 0x38 + i * 0x18);
        size = *(unsigned long *)(vas + 0x38 + i * 0x18 + 0x10);
        res = *(unsigned long *)(vas + 0xb0);
        lv = *(unsigned long *)(vas + 0xb8);
        if (*(unsigned long (**)(unsigned long,unsigned long,unsigned long))(lv + 0x30) == 0) {
            for (j = 0; ; j++) {
                if (size == 0) break;
                ent = (*(unsigned long (**)(unsigned long,unsigned long))(lv + 0x10))(res, ent);
                if (ent == 0x107 || ent == 0) break;
            }
        } else {
            ent = (*(unsigned long (**)(unsigned long,unsigned long,unsigned long))(lv + 0x30))(res, ent, size);
        }
        if ((ent & 0xff) != 0) { sk_err_a(ent, eb); return ent; }
        i++;
    } while (i != nreg);
no_regions:
    if (*(unsigned long *)(vas + 0x18) == 0 || (t == 1 && *(unsigned long *)(vas + 0x20) == 0)) {
        base = *(unsigned long *)(vas + 0x38 + nreg * 0x18);
        if (t != 2) {
            ent = (*(unsigned long (**)(unsigned long,unsigned long))(*(unsigned long *)(vas + 0xb8) + 0x10))
                        (*(unsigned long *)(vas + 0xb0), base);
            if ((ent & 0xff) != 0) {
                lv = sk_tb_meta(ent, eb);
                if (lv == 0) sk_err_b();
                else sk_vas_abort(0x5b6800);
            }
        }
        sk_pmm_free(base);
    }
    ent = (*(unsigned long (**)(unsigned long))(*(unsigned long *)(vas + 8)))(*(unsigned long *)vas);
    if ((ent & 0xff) == 0) {
        for (i = 0; i < 0x1a; i++) *(unsigned long *)(vas + i * 8) = 0;
        return ent;
    }
    return ent;
}
...
/*--------------------------------------------------------------------*/
/* FUN_00042abc @ 0x00042abc   (est. sk_spanmap_lookup)
 * Ghidra: long FUN_00042abc(long param_1,long param_2,long param_3)
 * Translates a virtual address through the spanmap interval tree at a given
 * level to obtain the physical frame, checking the per-CPU page-table cache
 * (DAT_006af2b0 +0xe8/0xf0) for a hit before walking. Returns the frame, or 0
 * if the translation hits a hole. Aborts on invalid level geometry.
 * Confidence: medium
 */
unsigned long sk_spanmap_lookup(unsigned long vas, long level, long va)
{
    unsigned long cnt, frame, pg, n, i, idx;

    if (vas == 0) { sk_x_004b3d2c(); return 0; }
    cnt = (va - *(unsigned long *)(vas + 0x38 + level * 0x18 + 8)) >> (level * 0xb + 0xe & 0x3f);
    if (cnt < *(unsigned long *)(vas + 0x38 + level * 0x18 + 0x10)) {
        frame = *(unsigned long *)(vas + 0x38 + level * 0x18) + cnt * 0x40;
        pg = sk_cpu_get(0x6af2b0, 4, 0xb);
        if (vas == *(unsigned long *)(pg + 0xf8)) {
            n = *(unsigned long *)(pg + 0xe8);
            if (n != 0) {
                idx = *(unsigned long *)(pg + 0xf0);
                for (i = 0; i < n; i++) {
                    if (*(unsigned long *)(pg + 0x88 + ((idx + i) % 6) * 0x10) == frame)
                        return *(unsigned long *)(pg + 0x88 + ((idx + i) % 6) * 0x10 + 8);
                }
            }
        }
        return frame;
    }
    sk_vas_abort(0x5b68a2);
    return 0;
}

/*--------------------------------------------------------------------*/
/* FUN_00042c20 @ 0x00042c20   (est. sk_spanmap_release)
 * Ghidra: ulong FUN_00042c20(long param_1,ulong param_2)
 * Releases (unmaps) a span range in the VAS. If the target is the kernel's own
 * root span it delegates to the spanmap op; if the level-2 config it is a
 * direct no-op; otherwise it validates the range against the span and issues the
 * span object's unmap method. Returns the resulting L4 error.
 * Confidence: medium
 */
...
/*--------------------------------------------------------------------*/
/* FUN_00042d04 @ 0x00042d04   (est. sk_spanmap_commit)
 * Ghidra: ulong FUN_00042d04(undefined8 *param_1,ulong param_2)
 * Commits an unmapped span range: validates that the target is not the kernel
 * root span, bounds-checks the range, then invokes the span object's unmap
 * method under the span lock. Returns the committed address or aborts.
 * Confidence: medium
 */
...
/*--------------------------------------------------------------------*/
/* FUN_00042ed0 @ 0x00042ed0   (est. sk_spanmap_op)
 * Ghidra: ulong FUN_00042ed0(ulong param_1,ulong param_2)
 * Core spanmap operation: unmap/remap a 16K-granule-aligned region by walking
 * the interval tree and issuing the span object's unmap method for each covered
 * span, retrying preempted ops. Handles the kernel root span specially with a
 * pmm lock; a failed alloc causes the op to be deferred on the pmm free list.
 * Confidence: medium (complex); large function condensed.
 */
unsigned long sk_spanmap_op(unsigned long va, unsigned long level)
{
    unsigned long u, v, span, r, res, eb[4];
    unsigned char t;
    unsigned int flags;
    ...
    v = sk_vm_present();
    if ((v & 1) == 0) {
        if (sk_vm_lock(u) == 0) sk_vas_stack_push();
    }
    r = sk_vm_obj_ref(u);
    span = *(unsigned long *)(u + 0xf8);
    v = level + 1;
    res = -1L << (level * -3 + v * 0xe & 0x3f);
    r = sk_spanmap_lookup(span, level, res & va);
    CallSupervisor(1);
    if (r != 5) {
        if ((r & 0xfb) != 0) { sk_x_004b3d54(r, eb); sk_vas_abort(0x5b6e4b); }
        t = *(char *)(span + 0x98);
        if (t == 2) flags = 0;
    ...
    sk_vm_obj_unref(u, 0, 0);
    return 0;
}

/*--------------------------------------------------------------------*/
/* FUN_000436fc @ 0x000436fc   (est. sk_vas_populate_level_nonself)
 * Ghidra: ulong FUN_000436fc(long param_1,undefined8 param_2,ulong param_3,
 *   ulong param_4, undefined8 param_5, undefined8 param_6, undefined8 param_7,
 *   undefined8 param_8)
 * Populates a VAS page-table level for a non-self mapping. For the kernel root
 * span it walks via the spanmap op; otherwise it validates the span geometry and
 * issues the span object's alloc method per region, retrying preempted faults.
 * Returns an L4 error code; aborts on bad geometry.
 * Confidence: medium
 * Notes: string "populate_level_nonself" (0x5b7444).
 */
unsigned long sk_vas_populate_level_nonself(unsigned long vas, unsigned long va, unsigned long level,
        unsigned long size, unsigned long a, unsigned long b, unsigned long c, unsigned long d)
{
    unsigned long u, v, span, res, eb[4];
    unsigned char t;
    unsigned int flags;
    ...
    if (vas != 0) {
        u = sk_cpu_get(0x6af2b0, 4, 0xb);
        if (vas == *(unsigned long *)(u + 0xf8)) {
            span = sk_spanmap_op(va, 1);
            return span != 0;
        }
        return sk_vas_populate_level_inner(vas, va, 1, size);
    }
    /* condensed: fallthrough to generic populate */
    v = sk_cpu_get(0x6af2b0, 4, 0xb);
    (void)v; t = 0; flags = 0; res = 0; eb[0]=0;
    return sk_vas_populate_level_inner(vas, va, 1, size);
}

/*--------------------------------------------------------------------*/
/* FUN_00043780 @ 0x00043780   (est. sk_vas_populate_level_inner)
 * Ghidra: ulong FUN_00043780(long param_1,ulong param_2,ulong param_3,ulong param_4,
 *   undefined8 param_5, undefined8 param_6, undefined8 param_7, undefined8 param_8)
...
/*--------------------------------------------------------------------*/
/* FUN_000533ec @ 0x0004dfcc   (est. sk_fault_startfault_ret)
 * Ghidra: thunk_FUN_000533ec
 * Retry helper for the fault-start path: re-enters the fault start logic.
 * Confidence: low
 */
...
cl4_result_t sk_tb_ph_range(void)
{
    ...
    u = sk_global_cfg();
    if (u <= u + 0x160 &&
        *(unsigned long *)(u + 0xa8) <= *(unsigned long *)(u + 0xa8) + *(unsigned long *)(u + 0xb0)) {
        r.lo = u;
        return r;
    }
    sk_vas_abort(0x5aed68);
 ...
 * Looks up a TB placeholder by name, copies its name into a buffer, and invokes
 * the callback. Returns whether the placeholder was found.
 * Confidence: medium
 */
void sk_tb_ph_name(unsigned long name, unsigned long len, unsigned long cb, unsigned long arg)
{
    unsigned long buf[34];
    unsigned long p, u;

    u = sk_x_00050c88(name, len);
    if (u != 0) {
 ...
 * Compares two 0x100-byte TB name buffers, returning -1/0/1.
 * Confidence: medium
 */
 ...
 * Same as sk_tb_cmp3: compares two 0x100-byte TB name buffers.
 * Confidence: medium
 */
 ...
 * Serializes a TB placeholder array into a destination buffer, resolving each
 * element's address through the placeholder and copying the payload, returning
 * the number of elements written.
 * Confidence: medium
 */
unsigned long sk_tb_ph_dump(unsigned long p, unsigned long ph, unsigned long region, unsigned long dst,
                            unsigned long len)
{
    unsigned long kind, esz, cb, u, i, n, base;
    unsigned long cnt;
    ...
    cnt = 0;
    n = len / 0x107;
    esz = *(unsigned long *)(kind * 0x30 + 0x65bf80);
    for (i = 0; i < n; i++) {
        u = dst + i * 0x107;
 ...
 * Serializes a TB placeholder array with a different record stride (0x287),
 * invoking the per-element serializer sk_x_00050ce8.
 * Confidence: medium
 */
unsigned long sk_tb_ph_dump2(unsigned long p, unsigned long ph, unsigned long arg, unsigned long dst,
                             unsigned long len)
{
    unsigned long kind, esz, cb, u, i, n;
    unsigned long cnt;
    ...
    for (i = 0; i < n; i++) {
        u = dst + i * 0x287;
        if (sk_tb_ph_resolve(ph, i) != 0) {
            cnt += sk_x_00050ce8(arg, sk_tb_ph_resolve(ph, i), u) & 0xffffffff;
        }
    }
    (void)p; (void)cb;
    return cnt;
 ...
 * Looks up a TB placeholder by name with a larger buffer (0x120) and invokes
 * the callback. Returns whether it was found.
 * Confidence: medium
 */
 ...
 * Compares two 0x100-byte buffers, returning -1/0/1.
 * Confidence: medium
 */
 ...
 * Serializes a TB placeholder array with 0x107-stride records, calling the
 * element serializer sk_x_00051488.
 * Confidence: medium
 */
unsigned long sk_tb_ph_dump3(unsigned long p, unsigned long ph, unsigned long arg, unsigned long dst,
                             unsigned long len)
{
    unsigned long kind, esz, cb, u, i, n;
    unsigned long cnt;
    ...
    for (i = 0; i < n; i++) {
        u = dst + i * 0x107;
        if (sk_tb_ph_resolve(ph, i) != 0) {
            if (sk_strcmp(sk_tb_ph_resolve(ph, i), u + 8, 0xe0) == 0) {
                sk_x_00051488(arg, sk_tb_ph_resolve(ph, i), u);
                cnt++;
            }
        }
    }
 ...
 * Serializes a single 8-byte TB placeholder element, invoking the callback.
 * Confidence: medium
 */
 ...
 * Resolves the physical addresses of a TB placeholder array (16-byte records),
 * writing each element's resolved address into the destination. Returns the
 * number of elements resolved.
 * Confidence: medium
 */
unsigned long sk_tb_ph_addr2(unsigned long p, long sel, unsigned long region, unsigned long ph,
                             unsigned long len)
{
    unsigned long u, i, n, rec, cnt, ph2;
    unsigned long cntn;
 ...
 * Builds a device-tree space mapping from a TB placeholder: allocates the
 * device-tree span, resolves the placeholder's elements, and writes the
 * resulting frame into each 0x18-byte destination record.
 * Confidence: medium
 * Notes: string "failed_to_create_device_tree_spa" (0x5bb844).
 */
unsigned long sk_tb_ph_dt(unsigned long p, unsigned long ph, unsigned long region, unsigned long dst,
                          unsigned long len)
{
    unsigned long u, i, n, cnt, frame, span, res;
 ...
 * Returns whether the TB placeholder resources are ready (region present and
 * non-empty).
 * Confidence: medium
 */
bool sk_tb_ph_ready(void)
{
    ...
    u = sk_global_cfg();
    if (u > u + 0x160) sk_vas_abort(0x5aed68);
    if (*(unsigned long *)(u + 0x78) == 0 || *(unsigned long *)(u + 0xa8) == 0) return false;
    return *(unsigned long *)(u + 0xb0) != 0;
}
 ...
 * name, and if the placeholder's kind has a relocation callback registered,
 * invokes it with the relocated segment base.
 * Confidence: medium
 */
void sk_tb_ph_reloc(unsigned long p, unsigned long ph, unsigned long arg, unsigned long seg,
                    long off, unsigned long name)
{
    unsigned long kind, cb, base, seg2, u;

    seg2 = sk_x_00051e5c(seg, name);
    kind = (unsigned long)*(unsigned char *)ph;
    if (kind > 0x1c) sk_panic(0x5bb75b);
    cb = *(unsigned long *)(kind * 0x30 + 0x65bf90);
    if (cb != 0 && *(unsigned long *)(kind * 0x30 + 0x65bf88) != 0 && seg2 != 0) {
        base = sk_x_00051ee4();
        if (base != 0) {
            u = *(unsigned long *)(base + 0x20) + off;
            if (u < u || u - u < *(unsigned long *)(base + 0x28)) sk_vas_abort(0x5aed68);
            (*(unsigned long (**)(unsigned long,unsigned long,unsigned long))(cb))(p, ph, arg);
        }
    }
}
 ...
 * Scans every loaded image's __PDATA / __DATA segments, applying the TB
 * placeholder relocations found in each. Walks the segment list and the
 * placeholder iterators.
 * Confidence: medium
 * Notes: strings "___PDATA" (0x5bb7ae), "__DATA" (0x5bb23b).
 */
void sk_tb_ph_scan(int do_pdata, unsigned int flags)
{
    unsigned long img, span, u, it[2], ph, cnt, base;
    cl4_result_t r;
        ...
        if (*(char *)(img + 8) < 0) {
            ...
            while (ph != 0) {
                if ((char)*(unsigned long *)ph == 0 && *(unsigned char *)(ph + 3) == 0 && *(unsigned long *)ph == 0) {
                    if (do_pdata) {
                        sk_tb_ph_reloc(u, ph, r.lo, base, *(unsigned long *)(img + 0x20), 0x5bb7ae);
                    }
                    if ((flags & 1) == 0) {
