 ...
 * and registers its embedded lock; panics otherwise. FUN_fffffe000b78fb24. */
void waitq_validate(uint64_t waitq);

/* XNU container/object-registry lookup by handle + type. The hypervisor uses
 * this (via hv_ikot_hypervisor_handler FUN_fffffe000b985e38) to resolve its
 * registered IPC kobjects. Returns 1 when a live object of `type` matches
 * `handle`, 0 otherwise. FUN_fffffe000b7e0d8c. */
 ...
 * FUN_fffffe000b95d6f4 (est. copyout). 100+ kernel callers each. */
 ...
 * FUN_fffffe000b7f0afc (lock), FUN_fffffe000b7f1e80 (unlock),
 * FUN_fffffe000b7f1e4c (variant). 100+ kernel callers; full futex/waitq
 ...
 * FUN_fffffe000b793cf4 (zone free w/ waitq teardown),
 * FUN_fffffe000b862b6c (refcount hash dec + free). Universal kernel. */
 ...
 * FUN_fffffe000b95fe60; 7 kernel callers; used by hv_caps_cpu_report. */
extern long cache_type_lookup(int idx);

/* Credential/sandbox entitlement probe (DAT_fffffe0007e93310 slot +0x1c0);
 * returns 0 when the cred carries the named entitlement. FUN_fffffe000c0f8cfc;
