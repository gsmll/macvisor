...
/*
 * vCPU object layout (0x100 bytes, est. from FUN_fffffe000b989390/b989040
 * and the hub FUN_fffffe000b989a44).
 *
 *   +0x00  hv_vm_container_t *container  (owner VM object)
 *   +0x08  void *guest_mem               (second allocation, EL2-mapped)
 *   +0x10  vm_map_address                (mapping handle / address)
 *   +0x58..  misc (pair consumed by hub exit accounting)
 *   +0x88  guest context / pending ptr
 *   +0xb0  void *el2_state               (per-vcpu EL2 save area, +0x4008.. reason)
 *   +0xc0  vmm_ctx *                     (has +0x1400)
 *   +0xd0  void *el2_extra               (0x4000 allocation)
 *   +0xd8  (bound-container scratch; x26 = vcpu+0xd8)
 *   +0xe0  attach id string / object ptr
 *   +0xe8  pending ptr pair (stp xzr,xzr)
 *   +0xf0  void *attach_obj              (container/descriptor; [..+8]=vm, +0x28,+0x30,+0x1008)
...
typedef struct hv_vcpu {
    ...
    uint64_t       guest_ctx;      /* +0x88 */
    uint8_t        _pad2[0x28];    /* +0x90..0xb8 */
    uint64_t       el2_state;      /* +0xb0 */
    uint64_t       vmm_ctx;        /* +0xc0 */
    uint64_t       el2_extra;      /* +0xd0 */
    uint64_t       scratch;        /* +0xd8 */
    uint64_t       attach_id;      /* +0xe0 */
    uint64_t       pending[2];     /* +0xe8 */
    uint64_t       attach_obj;     /* +0xf0 */
...
void hv_vcpu_object_release(void *object);
...
extern uint64_t vm_map_enter(uint64_t, uint64_t, uint64_t, uint64_t, uint64_t); /* b8b51c8 */
