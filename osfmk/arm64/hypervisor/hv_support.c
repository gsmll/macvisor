 ...
 * Seed roots: hv_support_init (est. hv_support_init),
 *   hv_el2_feature_detect (est. hv_el2_feature_detect),
 *   hv_available (est. hv_available / mach-trap dispatcher).
 *
 * The hv mach-trap operation table this tree dispatches through lives at
 * PTR_hv_op_table; its handlers are owned by the vcpu-core and
...
/* ------------------------------------------------------------------ *
 * hv_support_init @ 0xfffffe000b984d4c   (est. hv_support_init)
 * Ghidra: undefined8 hv_support_init(void)
 ...
 * feature detection (hv_el2_feature_detect) and returns 1 on success, 0 if
 ...
 *   bit 4). Kernel deps: kernel_dt_node_lookup (DT "/product" lookup),
 *   kernel_dt_prop_get (DT property get), kernel_boot_arg_get (boot-arg
...
static int hv_support_init(void)
{
    ...
    if (((cur_el & 0xc) == 8) && (hv_chip_id >> 0x18 == 0x61)) {
        ...
        if ((hv_chip_id & 0xffe0) != 0x200) {
            ...
            prop_val = 0;
            prop_size = 0;
            rc = kernel_dt_node_lookup(0, "/product", &dt_node); /* DT "/product" node lookup, kernel */
            if ((rc == 1) &&
                (rc = kernel_dt_prop_get(dt_node, "apple-isa-vm-quota",
            ...
            if ((hv_bootarg_flags >> 4 & 1) != 0) {
                ...
                boot_quota = 0;
                rc = kernel_boot_arg_get(hv_bootarg_table + 0x6c,
			...
			hv_el2_feature_detect();   /* FUN_fffffe000b987fa8 (same file) */
            return 1;
        }
    }
    return 0;
...
/* ------------------------------------------------------------------ *
 * hv_el2_feature_detect @ 0xfffffe000b987fa8   (est. hv_el2_feature_detect)
 * Ghidra: void hv_el2_feature_detect(void)
 ...
 * PTR_hv_op_table table (commands 0xd and 0x12 are reserved NULL
...
static void hv_available(long param_1)
{
    ...
    if (((byte)hv_trace_flag & 1) != 0) {
        kernel_trace(0x10c0015, cmd, arg, 0, 0); /* kernel trace */
    }
    ...
    if (hv_available_flag == 0) {   /* hv availability global */
         ...
         * PTR_hv_op_table; 0xd/0x12 are the two NULL entries. */
        result = (long)((hv_op_func_t *)(PTR_hv_op_table))[cmd](arg);
    }
    ...
    *(long *)(param_1 + 8) = result;
    ...
    kernel_trace(0x10c0016, result, 0, 0, 0); /* kernel trace */
}
